...
| `stages` | 作者(人工编辑)或系统(静态表/蒸馏自动回填) | 五角色归属,**可多值**\
(`code-review` 真的既属构建也属优化)。五个全挂 = 全阶段通用,对每个阶段的注入候选\
都算命中;空 = 见下一行。存在 `skill_stage` 关联表,**不是** skill 行上的一个值。 |
| `stage_origin` | **系统** | 归类**动作**的出处:空 = 还没人归类 / `table` = 随包\
静态表回填 / `distilled` = 按蒸馏出处 Issue 派生 / `manual` = 人工在 SkillHub 编辑面板\
改过(此后自动回填不再覆盖) / `legacy` = 搬自已被删除的旧 `skill.stage_ref` 单值列、\
原始归类出处不可考(真实案例:`metrics-render`,来自未合入本分支的另一条产品线,\
本分支的静态表覆盖不到它)。它与 `stages` 是否为空共同决定四态——空 `stages` + \
非空 `stage_origin` = **已判定「不属任何阶段」**(判过了,跟五阶段无关),空 + 空 = \
