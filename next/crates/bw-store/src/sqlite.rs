...
impl SqliteStore {
    ...
    pub async fn open(path: &str) -> Result<Self> {
        ...
        // next 切片七A 这里曾经有一段 `#[cfg(feature = "import")]` 应用
        // `schema_import.sql` 的代码;七-1 修复轮(task-s7a-review.md
        // Critical-1)删掉了它,连同那个 `import` cargo 特性一起——这个函
        // 数现在**结构上不知道**导入专用表的存在,不是「特性关了就不知
        // 道」。导入专用写入面搬进了独立 crate `bw-store-import`,它的
        // `ImportSqliteStore::open` 自己开一条独立连接、自己应用
        // `schema_import.sql`,只在那个 crate 被真正调用时才建那两张表
        // (`app-desktop` 永远不依赖那个 crate,见它的顶部文档)。这个函数
        // (`SqliteStore::open`,壳唯一会调用的开库路径)因此**永远不会**
        // 建出 `import_ledger`/`artifact_archive`,不管这次编译有没有把
        // 别的 crate 请求的特性统一进来——这里根本没有特性可统一。
