...
/// `pub`(不是 `pub(crate)`)——next 切片七-1 修复轮:`bw-store-import`
/// (独立 crate,导入专用写入面,见该 crate 顶部文档)要把 `MetricTier`
/// 编回 TEXT 列值,自己不重新发明一遍这份编解码。
pub fn metric_tier_text(t: MetricTier) -> &'static str {
    match t {
