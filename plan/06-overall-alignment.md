...
**即时可全真轨**(不等任何外部):
1. **A1 · Autopilot 建单**(规格已冻结,半天)
2. **A2 · 关联列 C**(小;为 A3 铺路,Done 边沿产物即挂 issue_id)
3. **A3 · RunIssue 结构**(A 的全部结构 + Mock 自检;含蒸馏技能注入=复利闭环)
4. **A4 · D+E**(Issue 观测喂入 + 交棒开放件检查——方法论环与看板的最后两道焊缝)
5. **A5 · F+H**(转移守卫/Blocked 原因/看板指派)

**等外部轨**(用户动作后立即接):
- **B1 · G4 网关闭合**(等 token 刷新;401 判定与步骤见交接件 P2)→ RunIssue/起草/聊天全真化
- **B2 · G3 起草真实 → B3 聊天真实回复**(依赖 B1)
