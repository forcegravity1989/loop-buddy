...
| 5.5 · 嵌入终端整体移植 | ✅ commit `7d8a2ac`+修 `78e69ff`(尺寸同步接回 v1 `resize()` 语义、握手自述纠偏并登记 §10 第 21 条上游依赖;opus 评审=移植保真度逐字节核过、三突变全红)。v1 `op.rs` 终端组件(xterm.js 渲染+按键编码表+UTF-8 跨批处理)+ 三个前端资源(随包内联,零改写)整体移植进 `app-desktop`;主控裁决 7 落地——字节抽取节奏(v1 同款 100ms)搬进引擎侧(`bw-engine::agentcli::pump`),壳只订阅不拉取(新增 `bw_connector::caps::Interactive` 能力 + `RunManager::terminal_feed`/`terminal_input`),`guard-shell-no-clock` 复核零开口;JS↔Rust 键盘/resize 桥接从 v1 的 30ms 轮询改成 Dioxus `document::eval` 双向 channel 的事件推送(壳零时钟这条硬约束逼出的接口适配,不是重写);逃生舱行保留,终端可用时并存;`terminal_feed_smoke` 指挥器(bw-app,新)headless 验证字节泵→订阅全链路(读方向已知序列逐字节比对/写方向 terminal_input 真送达/取消后如实回落);深链渲染证明照旧(`[BW_OPEN]`/`[BW_REAP]` stderr,零回归)。**如实标注未解除的边界**:壳本片仍不发起 `RunIssue`(§10 第 19 条那条限制原样成立),因此生产部署的 `bw-next` 进程今天永远不会有"自己启动的"活会话——本片交付的是"字节泵+订阅接口+终端组件"这条完整链路,验证手段是 headless 指挥器,不是真的在跑着的桌面壳里看见一个终端 |
| 六 · multica cli 穿刺与缝入 | ⬜(等用户提供仓库指针) |
| 终审收尾 · 全分支审查 + 减法专项 | ✅ opus 终审判「敢合」(旧工程零触碰护栏 git 实证;四铁律逐条到代码核过);1 Critical(v1 移植的无条件删目录,紧急修 6c5e2f9 改「只造不删」并登记对 v1 的有意分歧)+10 Important 全数处置(波二 bce647f/414755d/1507e99:CI 补线/台账对齐/契约分类恢复/ArchiveMetric 接线等 20 条);死代码审计+减法专项(波三 9811bee…7b39d14+收尾 6f3559b):bw-core 5064→1272 行,全仓净删 4000+ 行退役概念,十指挥器断言条数一条不差证明零活物被删;Minor 池 65 条分诊留档(必修 5 已修/建议 34/接受 18) |
| 七 · 真实项目切换 + 一次性导入 | 🔄 七-1(af7ba5e/6592a8e+修 05868f3/8e799e1:导入面独立 crate bw-store-import+导入器 44 断言进 CI)与七-2(be3092b/713df5e+修 b013205:开工闭环 issue_kickoff 88 断言进 CI,壳能真实发起开工/取消,§10 第 19/20 条清偿)收官;七-3 真库副本只读侦察完成(逐实体计数:项目 3/北极星 3/指标 31/观测 64/活 37/产物 764/交棒 2,拒绝清单空,脏数据六类核查全过)。**七-4 真写已在用户确认门点头后完成**:对旧库原件只看档核对计数与七-3 逐项相同(用户日常库三周未变)→ 对原件跑 `--confirm` 写进新壳默认库(`~/Library/Application Support/BuildersWorkbenchNext/workbench.db`,三次哈希/大小/mtime 比对逐字节相同,旧库全程只读);计划=实写=project 3/metric 34/observation 64/issue 37/handoff 2/artifact 764,跳过 0,导入流水 `import_ledger` 落一行。六段点亮验收(aihot 日报,两屏深链+sqlite 独立读回,详见 `docs/design-next/verify-s7-real-project.md`):①北极星/②五角色/③引领指标/⑤风险与决策(如实空)/⑥交付证据五段有真数据或如实空且有说明,**第④段当前 Loop 如实空**(未起任何真实会话,被台账第 6 条信任对话框问题挡住,路线留用户拍板);待人处理=15(8 条过期琥珀+6 条无观测+1 条评审中,与深链 stderr 逐位对上)。**按主控裁决 10 拆两半记账:七-主体(导入器+开工闭环+六段点亮五段)本次收官;七-验收里程碑(六段全链含第④段真会话+归档两步)继续挂在 §10 第 6 条信任问题上,如实标注、不算失败也不冒充完成** |
...
2. **`ExecSpec` 没有独立的「这件活要干什么」字段**(登记日 2026-08-10)
   ...
   - **登记日**:2026-08-10。**实况补记(2026-08-11,next 紧急修 I1,终审 Important-1)**:待办里的第二条路径已落地——`bw-connector::contract::TASK_BODY_LABEL` 定了这条调用约定,`bw-app::cmd::issue::run_issue`(`task_body_inject`)把活的标题+描述装进 `inject` 里 label 恰好等于这个常量的那一块,`bw-engine::agentcli::connector::AgentCliConnector::start` 首启时把它单独取出来当位置 prompt(不折进系统提示词),`issue_kickoff` 指挥器 K9 段读回验证到 `ExecSpec.inject` 这一契约边界(标题+描述原文都在)。**契约字段之争维持原登记,本条不裁决**:`ExecSpec` 没有加任何字段,`PROTOCOL` 没有变——「该不该直接给 `ExecSpec` 加一个任务正文字段」这一半待办依旧开着,只是「位置 prompt 装不装得上活正文」这个更紧急的产品缺口(终审 Critical 级别的姊妹条目 Important-1)已经不再成立。
...
19. **逃生舱在真实深链渲染里展示不出「活着的」进行中运行,要等 RunIssue 接入壳**(登记日 2026-08-11,切片五E 实施时深链验证撞见)
    ...
    - **登记日**:2026-08-11。
    - **2026-08-11 补记(切片 5.5 实施时,commit `7d8a2ac`)**:本条描述的边界原样成立,如实记一笔——嵌入终端(design-s5-hexpanel.md §5.3 搬运清单)在切片 5.5 整体移植进壳,字节泵/订阅接口(`bw-engine::agentcli::pump` + `bw_connector::caps::Interactive` + `RunManager::terminal_feed`/`terminal_input`)全部是生产代码、`terminal_feed_smoke` 指挥器 headless 验证过整条链路真的通(读方向已知字节序列逐字节比对、写方向 `terminal_input` 真送达、取消后如实回落到「没有」);但**壳依旧不发起 `RunIssue`**,这条边界一个字没动。因此嵌入终端组件(`screens::terminal::TerminalWidget`)在真实部署的 `bw-next` 进程里,今天与逃生舱同病:永远等不到一条「由这个进程自己开工、还活着」的运行,`RunManager::terminal_feed` 对任何深链渲染里能看到的运行行都会如实回 `None`——界面上不会多出任何东西(零回归,深链 stderr 复核过),但也不会真的显示出一个终端。这不是嵌入终端这一片的缺口,是本条缺口在终端这个新消费者身上的又一次复现;解除条件仍然是本条「待办」写的那件事(`RunIssue` 接入壳)。
    - **本条点名的结构性限制已消灭于 commit `be3092b`(七C · 开工用例进编排层)+ `713df5e`(七D · 壳接上开工)**:「壳不发起 `RunIssue`」这条前提本身不再成立——开工用例落地(`bw_app::cmd::issue::run_issue`/`cancel_run`)+ 壳接上「开工」「取消」按钮(`ShellCommand::RunIssue`/`ShellCommand::CancelRun` 转发进这两个函数,壳内零业务判断)。深链实测(`BW_DB=<临时库> BW_OPEN=<临时项目> BW_PANEL=hex ./target/debug/bw-next`,两次深链启动之间跑一次真实 `run_issue` 调用——同一段代码,壳按钮调的就是它):开工前 `运行=0`,开工后 `运行=1`,真实工作树在磁盘上造出来(`.git` worktree 标记可查,分支 `bw/issue-1`)。
    - **如实标注一个未完全等价的地方,不算过度声称**:上面这次实测的「开工」与「深链渲染」是**两个先后启动的进程**(`run_issue` 由一次性验证脚本触发,不是真的点了壳里的按钮),第二次深链启动时那条运行已经被 `reap_on_restart` 如实标成了 orphaned(`[BW_REAP] 重启收拾遗留运行 1 条`)——这是正确行为(design §3.1「不批量唤醒」),不是新缺口,但也意味着这次实测**没有**在深链渲染里看见一张「进行中」的逃生舱卡片,只证明了「运行=N」这个计数会跟着真实开工变化。要看见「进行中」卡片,开工与渲染必须发生在**同一个存活进程**里——这在这套桌面框架上今天只能靠真的点按钮触发(仓库已踩过两轮的结论:click 在这套 wry WebView 上被阻塞,`--real`/真实交互会话是它的验收路径,§10 第 6 条挡着,不在本片 mock 档范围)。这半句因此原样转移,不算本条缺口的一部分——本条缺口的待办只是「`RunIssue` 接入壳」,接完就消灭了。
...
20. **`BW_WORKSPACES` 深链变量本片未消费**(登记日 2026-08-11,切片五E 实施时如实登记)
    ...
    - **登记日**:2026-08-11。
    - **已消灭于 commit `be3092b`(七C)+ `713df5e`(七D)**:`bw_app::cmd::issue::run_issue` 新增 `workspaces_root: &Path` 形参,是这个深链变量的第一条真实消费路径——`project.root_path` 是绝对路径时原样用,是相对路径时接在 `workspaces_root` 后面解析(`resolve_main_workspace`,`bw-app/src/cmd/issue.rs`);`app-desktop::kernel::workspaces_root()`(同旧壳既有约定:`BW_WORKSPACES` 覆盖,不给就落数据库同目录下的 `workspaces/`)在壳的主循环里把它喂给每一次 `run_issue` 调用。`issue_kickoff.rs` 指挥器的 K2b 段专门覆盖这条路径(相对 `root_path` + `workspaces_root` 解析成绝对路径,断言真实拼出来的工作区路径以预期前缀开头)。
...
22. **产物证据的历史存档表(`artifact_archive`)建成之后,今天没有任何读者**(登记日 2026-08-11,切片七A/B 实施时按 `docs/design-next/design-s7-real-project.md` §12 开放问题 1 / 主控裁决 1 附带登记)
    - **现象**:一次性导入器(`bw-app/examples/import_legacy.rs`)把旧库「产物证据」表(v1 `artifact` 表,764 条真实数据)导进新库的 `artifact_archive` 表(`bw-store` `import` 特性,新表,design-s7-real-project.md §2.2 第⑥条),明确标注「历史存档、不再增长」——但界面/编排层今天**没有任何一处会去读这张表**。它当前的唯一价值是保全「哪件活登记了这个产物」(`issue_id` 列)这条历史关联,新工程的交付证据第三栏走的是读时现采(`bw_workspace::evidence::collect`),不查这张表,两条路完全不相交。
    - **来源**:design §12 开放问题 1 的选项 (c)(2026-08-11 主控裁决 1:按 (c) 办,不选「不导」)——「诚实,但要为它写读侧与界面文案,而它的读者今天不存在;等于为一张死表建一套展示」,裁决明确接受这个代价,理由是那 228 条(真库实况)issue_id 归属关联是「每件活的账目真实可查」的历史部分,重采(`git ls-files`)采不回来这条关联,丢它违背只追加精神。
    - **待办**:真需要「这件活当年登记过哪些产物」这条历史视图时,再决定读侧长什么样(独立小节 UI,还是并进交付证据第三栏当"历史"子栏)——不在切片七A/B 范围内动。
    - **登记日**:2026-08-11。

23. **旧库一批列在新库没有家,已如实丢弃**(登记日 2026-08-11,切片七B 实施时按 design-s7-real-project.md §2.2 各表「未导入列清单」汇总登记)
    - **现象**:一次性导入器跑完会在差异清单里打印这些列,但它们不会出现在新库任何地方——项目的远端身份三件套(`remote_path`/`remote_host`/`provider`)、`kind`/`descr`/`phase`/`cycle`/`benchmark`/`opportunity`/`allow_commands`、三个信号缓存列;指标的 `stage_kind`(阶段归属,已被第一处硬碰撞的改名处置吸收)、`amber_kind`/`amber_value`(黄灯带宽)、`last_target`/`driver`/`pos`、两个信号缓存列;活的 `github_number`/`pr_number`/`priority`/`assignee`/`blocked_reason`/`standard_skill`/`interactive_started`/`claude_session_id`。
    - **来源**:新工程范围缩小后这些概念要么退役(`kind`/`phase`/`cycle`/`benchmark`/`opportunity` 这类创建流程专属字段,新工程没有对应的创建体验)、要么改了住处(远端身份住连接器绑定,连接器今天不落库;上游会话号住运行行,运行不在导入范围内)。
    - **待办**:逐条在真实使用中确认「真的不需要」还是「需要但还没建」,后者按需补(补的时候记得配迁移双守卫,存量库加列不能只改 `schema.sql`)。
    - **登记日**:2026-08-11。

24. **旧库里能续接的会话,在新库接不回来**(登记日 2026-08-11,切片七B 实施时登记)
    - **现象**:旧库把上游会话号记在 `issue.claude_session_id` 上;新库把上游会话号记在 `run.upstream_session`(design-s4-runmanager.md §2.2)。计划 §5.2 点名的七类实体不含「运行」,导入器因此从不碰 `run` 表——旧库里那些理论上还能 `--resume` 接回去的会话,导入之后在新库里找不到对应的运行行,接不回来。
    - **来源**:`docs/design-next/design-s7-real-project.md` §2.2 第⑤条「上游会话号这一条要单说」+ 台账第 3 条(已登记同一条道理:R6 完整轮询链路结构性造不出来)的姊妹缺口——运行是过程性事实,不是「只此一份、丢了就没了」的不可再生事实,本稿判定为不导。
    - **待办**:真要接回,需要先决定运行历史导不导——目前判定为不导;若产品口径改变,需要另起一片专门做运行历史的导入(七类实体之外的第八类)。
    - **登记日**:2026-08-11。

25. **导入后每个项目都会有一棒,新库「尚未开棒」这个状态在导入数据上永远不出现**(登记日 2026-08-11,切片七B 实施时登记)
    - **现象**:旧库 `project.active_stage` 是 `NOT NULL DEFAULT 'prototype'`,新库 `project.active_stage` 可空(`NULL` = 尚未开棒,design-s5-hexpanel.md §2.1「不默认」原则)。导入器一律按旧值给数字(`import_legacy.rs` `parse_stage_text`),所以旧库里的项目导进来之后,新库里这一列永远不是 `NULL`——「尚未开棒」这个真实状态只会出现在**新建**的项目上,导入数据里天生没有这一档。
    - **来源**:两边的建模差异(旧库把「还没开始」和「在原型阶段」合并成同一个默认值,新库把两者拆开)。
    - **待办**:无(如实标注即可);新建的项目仍然会正常出现「尚未开棒」,这不是一个需要修的缺口,只是一个需要被记住的事实,免得被误读成导入器漏做了什么。
    - **登记日**:2026-08-11。

26. **执行连接器不落库,壳启动时按约定装配**(登记日 2026-08-11,切片七A/B 实施时按 design-s7-real-project.md §1.3/§11 第 5 条预先登记——本条对应的壳装配代码不在切片七A/B 范围内,登记先于实现)
    - **现象**:切片七的开工闭环(design §1)要求壳启动时给每个有本地检出根的项目装配一条执行连接器,但这份登记不落库——新库没有连接器表,项目仓那份采集连接器正本(`.bw/connectors.toml`)的词表今天只认脚本一种。切片七A/B(本次)完全不碰这一段(不涉及运行管理器/壳),仅按设计稿要求提前登记,避免后续切片(C 开工用例/D 壳接线)重复发现同一件事。
    - **来源**:`docs/design-next/design-s7-real-project.md` §1.3 三条如实标注 + §11 第 5 条。
    - **待办**:真需要第二家 agent 类 CLI、或一个项目挂两条执行连接器时,一并决定「登记住库里还是住项目仓正本」——这是切片七C/D 的事。
    - **登记日**:2026-08-11。

27. **采集连接器正本(`.bw/connectors.toml`)在新工程没有读取路径,自动采集不通**(登记日 2026-08-11,切片七A/B 实施时按 design-s7-real-project.md §3.2/§11 第 6 条预先登记)
    - **现象**:真实项目上的观测今天只能来自「一次性导入的历史」与「界面手填」——没有任何代码会去解析 `.bw/connectors.toml` 并跑一次自动采集。
    - **来源**:解析器没有从 v1 搬过来、词表不认 agent 类 CLI、自动采集还缺一个时钟(design §11 第 6 条,时钟该住哪要和 Loop 一等对象一起想,是本片之外的题目)。
    - **待办**:与 Loop 一等对象那一片一起做,不在切片七A/B 范围。
    - **登记日**:2026-08-11。

28. **新工程的命令面上没有建活入口**(登记日 2026-08-11,切片七A/B 实施时按 design-s7-real-project.md §3.1/§11 第 7 条预先登记)
    - **现象**:命令面(`bw-app` 的 `Command` 枚举)与桌面壳今天都没有「建一件新活」的路径。一次性导入器带来的旧库存量活(§2.2⑤)是新库里活的唯一来源。
    - **来源**:切片七刻意的减法——导入带来的待办活(真库实况:aihot 2 件、个人画像 2 件)足够把开工闭环跑通,不需要现建一条建活命令。
    - **待办**:做创建体验那一片时一并建;在此之前,任何新项目如果没有经过导入,新库里永远不会凭空长出一件活。
    - **登记日**:2026-08-11。

29. **工作树只造不删这条既有缺口(台账第 14 条),在真实项目上会第一次真的开始积累**(登记日 2026-08-11,切片七A/B 实施时按 design-s7-real-project.md §11 第 8 条预先登记——本次尚未产生任何真实工作树,提前记账)
    - **现象**:台账第 14 条已经记过「工作树只造不删」这条设计取舍;切片七A/B(本次)不涉及开工闭环,不会真的造出任何工作树。这条登记是提前占位——等到切片七C/D 的开工用例接线、真实项目上开始有人点「开工」,第 14 条描述的积累现象才会第一次真的发生。
    - **来源**:design-s7-real-project.md §11 第 8 条(呼应台账第 14 条,同一件事在真实使用场景下的复现)。
    - **待办**:同第 14 条——做一条显式清理用例,判据必须包含「这条路径上没有活着的会话」。
    - **登记日**:2026-08-11。

30. **代号登记表(`docs/code-schemes.md`)还没有把「切片七」系列登记进去**(登记日 2026-08-11,切片七A 实施时如实登记——按 `CLAUDE.md`「代号先登记再使用」要求,本条同时也是登记动作本身)
    - **现象**:本次两个 commit 的标题前缀用了「next 切片七A」「next 切片七B」——`docs/code-schemes.md` 此前的登记表(design §5.3 引用过)还停留在「切片一~五」;切片六、切片 5.5、切片七都是在登记表补齐之前就已经在用的前缀,这与文档正文承认的「登记滞后于使用」是同一类事实(历史上 P/S/W/R/L/A 六个字母都发生过同字母撞车,根因正是没有登记表)。
    - **来源**:`CLAUDE.md`「代号先登记再使用」的规矩 + design-s7-real-project.md §5.3/§11 第 9 条。
    - **待办**:下一次触碰 `docs/code-schemes.md` 时(计划落在设计稿 §8.2 建议的 E 提交,真实项目切换那一片),把登记表扩到切片七,并补上切片六、切片 5.5 这两条已经在用但还没登记的空缺。
    - **登记日**:2026-08-11。**已消灭于本次「next 切片七E · 真实项目导入落库 + 六段点亮验收留证」commit**:`docs/code-schemes.md` 那一行「切片一~五」已扩成「切片一~七(含 5.5)」,补齐了切片六、切片 5.5、切片七三个此前已在用但未登记的前缀。
...
32. **六段全链第④段(当前 Loop)在真实项目上仍然如实空——不是本次漏做,是台账第 6 条还没解除**(登记日 2026-08-11,切片七E 实施时按 `docs/design-next/design-s7-real-project.md` §3.3/§4.3 与主控裁决 10 登记)
    - **现象**:切片七E 对用户真实日常库跑了真写(三个项目导进新壳默认库,计划=实写、跳过 0)与六段点亮验收(aihot 日报,两屏深链 stderr + sqlite 独立读回,详见 `docs/design-next/verify-s7-real-project.md`)。①②③⑤⑥五段各自有真数据或如实空且有说明文案,**唯独第④段(至少一条本进程自己开工、走完整生命周期的运行)没有点亮**——按硬约束本次没有起任何真实会话,`SELECT COUNT(*) FROM run` 挂在 aihot 下读回为 0。设计稿 §3.3 原话「第④段做不到时,不算跑完」字面成立,但这正是主控裁决 10 已经预先拆开的两半记账:「七-主体」(导入器+开工闭环+六段点亮五段)可先收官,「七-验收里程碑」(六段全链含第④段+归档两步)挂在信任问题上。
    - **来源**:`docs/design-next/design-s7-real-project.md` §4(信任对话框六条已核实事实 + 三条候选路线,主控裁决 7 明言「不拍板,如实上呈用户」)与主控裁决 10(「`--real` 失败口径:切片七拆两半记账」)。
    - **待办**:等用户在 §4.2 三条候选路线(甲·预置信任 / 乙·认画面应答 / 丙·让人接一次,及「集中工作树目录、只信任一次公共父目录」的甲丙折中观察)之间拍板之后,再跑 `cargo run -p bw-app --example issue_kickoff -- --real` 与真实的开工闭环,把第④段点亮;点亮之后才具备归档旧应用第一步的触发条件(§8.2 提交 F,依赖 E 的六段验收通过)。
    - **登记日**:2026-08-11。

33. **造工作树时,`provision_issue_worktree` 对「目录已存在但不是合法工作树」的处理,与 v1 原实现有意分歧——v1 整删重来,BW 改为如实失败**(登记日 2026-08-11,终审 Critical-1,next 紧急修实施时登记)
    - **现象**:`next/crates/bw-workspace/src/provision.rs` `provision_issue_worktree` 造工作树前先看兄弟目录(`<主工作区的 parent>/<主工作区目录名>-issue-<活编号>`)在不在;在、但没有 `.git`(不是合法工作树)时,**修复前**的代码是 `let _ = std::fs::remove_dir_all(&sibling)`——不问青红皂白整删重来,删失败还被 `let _ =` 吞掉,错误不报、删除不留痕。这条路径落在**用户项目根的兄弟目录**上(不是 BW 自己的数据区),名字是可预测的确定性命名,用户自己在旁边建过同名目录(笔记/手工克隆/别的工具生成/`.git` 文件被清掉的旧工作树)完全可能——一次点击「开工」就会触发的无条件递归删除,直接违反 `CLAUDE.md`「破坏性永不自动」这条产品铁律。终审报告把这一条判为唯一的 Critical:切片七-4 是新工程第一次真的让用户项目目录进场,这条路径因此第一次真正进入射程。
    - **来源**:`crates/bw-engine/src/workspace.rs`(v1)逐字节移植件——旧工程 `crates/bw-app/src/lib.rs` 的生产路径同样调得到这段代码,同一个坑,不是这次移植新引入的,是移植前就带着的既有缺陷(design-s5-hexpanel.md §6.2/§6.4/§9 的搬迁范围只点名「函数体不改」,没有单独审过这一条分支)。按 plan/23 §0 第五轮定位推论③「移植件里发现的缺陷附证据登记,可回报伙伴修 v1,我们不擅改 v1 分支」,这条属于该登记、该回报的那一类。
...
38. **`bw-core` 是整包移植,不是「移植了三个器官」——约 1,500 行服务于已退役功能,零消费者**(登记日 2026-08-11,终审 Important-10,只登记不删,指向波三专项)
    ...
    - **登记日**:2026-08-11。
    - **清偿(波三,2026-08-11,commit `9811bee`+`3d34b5e`+`c36ba24`)**:七个整文件(1,481 行)全删——`bw_library`/`skill_spec`/`skill_body` 三件复利链零件重新逐符号 grep 复核,`next` 从未接出消费它们的 canon 构建器/剧本渲染系统,判定为「等复利链那一片接线」的前提本身不成立(接线对象不存在),改判直接删,9 处 `include_str!` 指向的 `docs/skills/*/SKILL.md` 文档原样保留。`model.rs` 里的退役实体这次也一并动了(2300→302 行):逐符号 grep 复核零外部消费者后,WorkflowSpec 构建管线/Hub 资产族/Cron 族/旧 Connector/旧 Artifact/旧 Issue/sessions·workflow 杂项/StageKind 四孤儿方法全删,只留 `Signal`/`SourceKind`/`StageKind`(主体)/`Cadence`/`IssueStatus`/`RunState` 六个有真实消费者的类型;`playbook.rs`(470→65 行)只留 `PlaybookCtx`/`render`;`ids.rs`(143→44 行)删 9 个死 ID。`bw-core/src` 总行数(含 `derive/`):删前 5064,删后 1272。随手清了 `bw-engine`(`RunCtx.workflow` 死字段 + `terminal_manager` 四死访问器 + `ConversationMeta` 三个只写字段)、`bw-connector`(`upstream/github.rs` 六个换落点前旧函数 + 自查追加的 P1 孤儿辅助件,875→672 行)、`bw-workspace`(`MetricTier::as_str`)。十指挥器断言条数与波二基线一条不差(hex_readback 113/issue_kickoff 92/import_legacy 45/run_races 196,其余六个照跑照绿),`cargo check -p bw-core --target wasm32-unknown-unknown --no-default-features` 仍绿。
