...
| 路径 | 是什么 | 谁在维护 |
|---|---|---|
| [`v1-prototype/`](v1-prototype/) → [`v2-prototype/`](v2-prototype/) → [`v3-prototype/`](v3-prototype/) | **当前迭代线**:V1 产品化(纳入项目简化 / 交互式指标环 / 内嵌终端 / 总览重构)→ V2(调度统一 / 最简多人)→ V3(原型进度页内嵌 Open Design)。每个目录自带 README 与逐文件状态表;`v1-prototype/LEFTOVERS.md` 是**遗留问题的唯一完整清单** | 伙伴会话(V1/V2/V3 分支已全部合入 main) |
| [`BACKLOG.md`](BACKLOG.md) | 减负重构缓做的冗余功能与结构债清单 | 本次会话建;做掉一条打一个 ✅ |
...
| [`skills/*/SKILL.md`](skills/) | `crates/bw-core/src/bw_library.rs`——BW 自带的 9 个基础技能包正本(evidence-first / spec-to-tests / baseline-before-touch / fresh-eyes-funnel / breaking-drill / competitive-analysis / north-star-discovery / metrics-binding / metrics-render) |

`buddy/README.md` 有这批资产的自述。**不要搬这两个目录**——路径是编译期硬绑定。

### 3. 历史档案([`archive/`](archive/),只加不改)

早期路线选型(`plan/00-05`)、做完即历史的执行批次(`plan/09-12,14,17-19,21`)、交接记录与 aihot 践行日志(`iterations/`)、superpowers 实施计划原件、Rust 重写前的 HTML 原型稿(`design/`)、2026-07 的演示报告与动图(`verification/`)。规则与目录见 [`archive/README.md`](archive/README.md)。**编号语义保留**:源码注释里的 `plan/09 §2` 去 `archive/plan/09-…` 找。
