...
> 指标体系对齐 / `collect.kind` 五值枚举 / 自动化免疫检验等自建强项),改
> 编只合入其判据与护栏结构,不是整体替换——测评全文与盲测证据见
...
>
> **自动化免疫检验**(与上面的虚荣指标黑名单并列,是比黑名单更上位的原
> 则——黑名单只挡得住"看起来像虚荣指标"的那几种,这条挡的是"任何看起来
> 正当、实则会被自动化悄悄刷绿"的指标)。起草完北极星,问一句:**假如给
> 这个项目挂一条永不失败的定时任务,这条北极星会不会永久满分?** 会 →
> 它量的是系统在运转,不是用户在获益,退回重写。
>
> 反例(真实):某内容日更类项目现行北极星曾定为「连续 N 天,每天都真实
> 产出一份满足数量下限的内容」,精确落进"创作产出"这个合法的北极星类
> 别,上面的黑名单一条也拦不住它——但只要给它挂一条每日定时任务,从上
> 线当天起这条北极星就永久满分。cron 只保证"跑了",从不保证"有人读、
> 有人因此变好"。
>
...
1. **落位**——精确落在使用/价值段,不是供给段(见下方工作步骤第 2 步的三段拆解)。
2. **自动化免疫**——挂一条永不失败的定时任务会不会让它永久满分(见上方硬性约束的反例)。
...
2. **拆三段:供给 / 使用 / 价值**:动笔起草北极星之前,先把这个项目拆成
   ...
   时间、少踩了坑、多赚了钱、少受了苦……)。**北极星必须落在第二段(使
   用)或第三段(价值),落在第一段(供给)一律退回重写**——供给段的指标
   回答的是"我们干了多少活",不是"用户过得好不好",这两件事经常被偷换。
   反例(真实):某项目自述痛点是"信息**过多**",痛点明明在价值段(用户
   被信息淹没、省不下时间),但它的北极星"连续每天产出一份内容"整条量
   的是文件有没有被生产出来——卡死在供给段,答非所问。
3. **起草北极星**:恰好一条,对应"这个项目存在的理由"。写清 `name` +
   `def`(精确到"怎么算作达成")。按上面的硬性约束,拒绝虚荣指标候选;并
   对照上一步的三段拆解——**明确指出这条北极星落在使用段还是价值段**,
   落不进去就还没起草完。
4. **起草滞后指标**(0..N,建议 2-4 条):结果性、滞后于动作才看得出好坏
   ...
   反例(真实):某项目定过一条命中量下限,而它自己 telemetry 里的真实
   观测值常年比这条下限高出一个数量级,这条下限形同虚设;项目自己的迭
   代记录里还写着优化阶段特意把输出量砍掉大半,说明这个产品的真实痛点
...
**逐指标强制判定**(第 4/5 步归类前先过这条判定题,不凭直觉贴标签):
...
| 阈值真实 | `target` 是否用项目真实历史数据校准,不是拍脑袋(同第 4 步阈值校准要求)? |
...
无固定 schema(人读文档,不被机器解析),但必须真实反映第 8 步列出的四块
