...
>
> 反例(真实):aihot 现行北极星「连续 7 天,每天都真实生成一份包含 ≥5
> 条命中关注面的摘要」,精确落进"创作产出"这个合法的北极星类别,上面的
> 黑名单一条也拦不住它——但只要给它挂一条 GitHub Actions 每日定时任务,
> 从上线当天起这条北极星就永久满分。cron 只保证"跑了",从不保证"有人
> 读、有人因此变好"。
>
...
2. **拆三段:供给 / 使用 / 价值**:动笔起草北极星之前,先把这个项目拆成
   ...
   反例(真实):aihot 自述痛点是"AI 热点圈信息**过多**",痛点明明在价值
   段(读的人被信息淹没、省不下时间),但它的北极星"连续 7 天每天生成一
   份摘要"整条量的是文件有没有被生产出来——卡死在供给段,答非所问。
...
4. **起草滞后指标**(0..N,建议 2-4 条):结果性、滞后于动作才看得出好坏
   ...
   反例(真实):aihot 定"命中数 ≥5"这一条下限,而它自己的
   `digests/telemetry.json` 真实值是 `raw:295 / hit:97 / items:29`——命中
   数常年远超 5,这条下限形同虚设;`docs/regression.md` 还记录着优化阶段
   特意把输出从 111 条砍到 30 条,说明这个产品的真实痛点是**上限**(别让
   用户被灌太多),指标却守着**下限**,方向完全反了,等于没守。
