...
- **预算/超时(2026-08-06 review 校准,原文口径已过时)**:交互式没有 per-token 硬 cap(`--max-budget-usd` 只配 `--print`),**也不靠超时兜底**——V1 真实走的是 Windows PTY 路径 `run_skill_pty`,那条路径压根没有 wall-clock deadline,只在会话 EOF / App 丢掉输入端 / 用户取消运行时收尾;1 小时 wall-clock 只存在于非 PTY 的系统终端 fallback(`run_skill`/`wait_child`),而且到点只是「宣告 completed」并不 kill claude。**这是有意的**(用户 2026-08-06 定):交互式是 user-in-loop,claude 就嵌在工作流看板的终端里,在干什么、跑多久人眼看得见,要停点取消即可(run 取消 → 轮次 future abort → PTY 会话 teardown,ConPTY Job kill-on-close 带走子进程树)。对 CLAUDE.md「单次花费封顶」是**显式偏差(已接受)**:封顶是防后台 runaway 的,后台 one-shot 轨(`ClaudeCliExecutor`)照旧 `--max-budget-usd 0.5` + `ATTEMPT_TIMEOUT_SECS`,一行没动。
...
5. **dev 偏差**:#1 `--prefill` **已解决(subagent 验)**——dev 用位置参数 prompt 是**对的**(= orca `promptInjectionMode:'argv'` 主路径,`claude "<prompt>"` auto-submit,文档化稳定);`--prefill` 是 orca 草案路径(预植入输入框、回车前审阅),buddy 不需要,代码里 `draft_prompt_flag:"--prefill"` 字段多余可清。#2 见上(resume 重设计)。#3 预算偏差接受(用户"无所谓";`--max-budget-usd` 确认只配 `--print`)——**口径 2026-08-06 校准**:兜底从「wall-clock 超时」改成「人在看板可见可停」,见 §2.5 预算/超时那条。
...
- **R1 预算封顶**:`--max-budget-usd` 只配合 `--print`(claude help 明示),交互式无 CLI flag 级封顶。**退路不是超时**(2026-08-06 review 校准,详见 §2.5 那条):PTY 路径没有 deadline,兜底是**人在工作流看板里看得见 + 随时取消**;wall-clock 只留在非 PTY fallback 且不 kill。orca 先例(信 flag + jsonl usage + 进程级 deadline)不再作为 buddy 的对照——buddy 选的是「人盯」不是「机器掐」。
