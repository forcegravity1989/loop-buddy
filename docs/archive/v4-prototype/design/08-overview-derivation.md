...
Builder 在项目栏点进一个项目,默认落在总览。一列横块从上到下:先看名片 + health——一眼知道这个项目要不要管;紧跟着北极星,再紧跟着本周计划进度条 + 运作活①②③三个状态点(一个「去计划 →」按钮带人到计划屏细看)。**头三块连起来读就是一条线**:这项目是什么 → 顶层目标是什么 → 这周在往那个目标上推什么(2026-08-21 把「本周计划进度」从倒数第二块提到这里,就是为了这条线)。再往下才是滞后、引领两层指标,每张卡下面挂着「本周谁在推它」的活——它们是展开看的细节,所以让到后面;然后是纯只读的代码仓统计;最后是在研版本与发版记录。**老项目没有额外的块**——回填出来的历史周文件与历史发版行和新项目的产出同格式,⑦块的发版记录里混排(带「回填」小字),历史周在计划屏左栏的「历史周」分组里,总览与计划屏都不做特殊处理(定,待拍-27 改写)。
...
**没有周计划时**:判据是**当前周的 `.bw/plan/YYYY-Www.md` 文件在不在**(不是「周列表里有没有本周」——本周永远在列表里)。文件不在时顶部出横幅,横幅分两态:①这周的运作活①还没建过 → 「本周还没有计划 → 开始本周」,点了就建运作活①、跳到会话屏 ▶开工、并顺手把刚起的那场会话选中(和其它活走同一条命令路径,总览的按钮只是最顺手的入口,不是唯一入口);②运作活①已经在路上(文件要等它产出、走 MR、人合入才有)→ 不再给按钮,换成一句「已开工 · {状态}」加一条去会话屏的路。**再点一次会被挡住**,那不是错误,是幂等。
...
**圆圈号是块的编号,不是屏上的先后。** 它在本篇和 09 / 13 / 14 各篇里被反复引用(比如「08 篇⑤」指的就是代码仓级那块),所以**不随排版调整改号**;屏上的实际先后看下表第一列,正本是 `crates/app-shell/src/screens/overview/mod.rs` 里那一串调用。下面的 §2.3 仍按编号顺序逐块展开,方便按号查。

| 屏上第几 | 编号 | 块名 | 点灯吗 | 主数据源 | 刷新时机 |
|---|---|---|---|---|---|
| 1 | ① | 项目信息 + health 面板 | health 灯是,名片本身不是 | 仓文件(`.bw/PROJECT.md`、`.bw/project.toml` 的 `[chat]` 段)+ 库(`project` 表既有列,仅作名称/想做什么/对标/北极星一句的显示缓存)+ 现算(health) | 每次打开总览现读 + 现算 |
| 2 | ② | 北极星 | 是(自己一枚 Signal) | 仓文件(`.bw/metrics.toml` 定义)+ 读数(今天只有周计划文件「本周指标读数」段这一路;新落点见 [14 篇](14-metrics-collection.md) §2.5) | 每次打开总览现读 |
| 3 | ⑥ | 本周计划进度 | 否 | 仓文件(`.bw/plan/YYYY-Www.md`,正本)+ 库(`issue` 缓存表,离线快速渲染) | 每次打开总览现读 + 查缓存 |
| 4 | ③ | 滞后指标(≤3) | 是 | 同②,取 `.bw/metrics.toml` 的 `[[lagging]]` 数组 | 同② |
| 5 | ④ | 引领指标(≤3) | 是 | 同②,取 `.bw/metrics.toml` 的 `[[leading]]` 数组 | 同② |
| 6 | ⑤ | 项目指标 · 代码仓级 | 否(只是现状数) | 现算(git 子进程 + 远端 `gh`;**不走 codegraph**) | **只在人点「↻ 立即采集」时采**,打开总览不自动跑 |
| 7 | ⑦ | 在研版本与发版记录 | 否 | 仓文件(`.bw/project.toml` 的 `current_version`、`.bw/releases.md`)+ 库(`issue.version` 列,活挂哪个版本) | 每次打开总览现读 |
...
| 现值 | **今天只有一路**:取当前周 `.bw/plan/YYYY-Www.md`「本周指标读数」段里指标名精确匹配的那一行(`build_metrics`,`crates/app-shell/src/bridge/vm_panels.rs`)。原来这里写的「按 `collect.kind` 分两路」已被 [14 篇](14-metrics-collection.md) §2.1 取代——分类判据不是采集方式,是**这条指标的历史现在还能不能重新算出来**:A 类现算、一个字都不存;B / C 类落 `.bw/metrics/readings.jsonl`。两类都还没接 | 每次打开总览现读 | 拿不到 → 「—」+「无观测 · Unknown ≠ 绿」 |
| 保鲜期 | 固定 `Cadence::Weekly`(V4 决定,见 3.3)。**卡上还没有这一栏**,见 [`../../LEFTOVERS.md`](../../LEFTOVERS.md) V4D-2 | — | — |
| 趋势 | **还没做**:`MetricCardVm`(`crates/app-shell/src/vm.rs`)今天没有趋势这一栏,业务指标卡上不画线。做法按 [14 篇](14-metrics-collection.md) §2.1/§3.4——A 类按窗口现算出完整序列(要几周算几周,不需要攒历史),B / C 类回扫 `.bw/metrics/readings.jsonl`,缺的周**断开、不补前值**。原来写的「回扫 8 份周计划文件」随读数落点一起作废 | — | — |
...
**2026-08-21 整块换了形态**:原来是一格格的小卡片,现在是**一行小字 + 三条近四周走势**。理由是那几个存量数回答的是「此刻有多少」,没有走势可言,不值得一人占一张卡片;真正值得画的是每周的变化。

**一行小字**(全部来自 `bw_engine::evidence::collect()` 与 `bw_v4::git`,只读子进程):累计提交、跟踪的文件、没提交的改动、`docs/` 下的 `.md` 篇数、首次提交日期、打过的标签数。每一项都标注从哪采的。

**三条近四周走势**(`bw_v4::trend::recent_weeks`,**一个数都不存**):每周提交、每周合入、每周合入的 PR。前两条按周窗口问 git,第三条真连远端(GitHub 走 `gh api search/issues` 按合入日期过滤;codehub 今天没有「按周窗口查 MR」的命令,那条线**如实留空**,不猜一个参数填上去)。这是 [14 篇](14-metrics-collection.md) §2.1「能采到今天,就能采到过去任意一周」那条判据的第一个真实落点:新接入的项目第一次采集就有完整四周,不用先攒。采不到的点**断开,不补前值**——补了就分不出「没变」和「没数」。

**不点灯、不上卷进 health**——工程虚荣指标,和业务指标(②③④)分层,`CONTEXT.md`「业务指标 / 项目指标」两层区分原样沿用。**也不走 codegraph**:原来这里写的「与 codegraph 索引(有就用,没有跳过)」没有实现,代码里一处都没调。

刷新时机:**只在人点「↻ 立即采集」时采**。打开总览不自动跑——采一次要在项目仓里跑好几条 git 再加一次远端往返,那不该在每次进项目、每次重绘时发生。也不带定时自动刷新(见第 6 节)。

读回:

```bash
git -C <workspace> rev-list --count HEAD; git -C <workspace> ls-files | wc -l
cargo run -p bw-v4 --example trend_smoke -- <workspace> <owner/repo>   # 三条走势逐周复算
# 复算提交数别用 `git log --pretty=format:%H | wc -l` —— %H 末行不带换行符,每次少数一条;用 rev-list --count
```
...
- ⑤块自动刷新——**只有手动「↻ 立即采集」这一条路**:打开总览不自动跑,也不额外起定时任务采集仓统计(和采集业务指标的定时任务是两回事)。
...
3. ~~**⑤块刷新频率**~~ —— **已定(2026-08-21)**:收成「只在人点『↻ 立即采集』时采」一条路,打开总览不自动跑。这一块现在还要打一次远端(合入的 PR 那条线),打开就跑等于每次进项目都做一次网络往返。本机缓存与定时后台刷新都不做,理由同 [14 篇](14-metrics-collection.md) §2.8 末段:与其猜「什么时候该重算」,不如把那颗按钮摆在人眼前。
