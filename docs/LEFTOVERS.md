...
| **V4A-4** | 规范铺底只做了第 1 步(buddy 自己写核心件)。第 2 步「合并调整」与第 3 步「历史回填」都没起 agent 会话 —— **两份剧本(`merge-adjust` / `asset-audit` 的 `mode=first`)B 刀已经写好并铺进项目仓了,缺的是「探测到之后自动起一次会话」那段编排** | C 刀 | 那张活的说明里已如实列出没跑的步骤,不假装做过 |
...
已关闭、已接受的条目仍在下方史实里，表里不重复。**减负-N** 系列是 2026-08-17/18 减负重构会话从 `docs/BACKLOG.md` 并入的（该文件已删，序号沿用；1、2 两条已做，收据在下方「减负重构收据」）。**V4A-N** / **V4B-N** 系列分别是 V4 A 刀(骨架 + 数据 + 主环)与 B 刀(运作活 + 会话屏)交付时如实登记的没做完的部分,代号已进 `code-schemes.md`。
