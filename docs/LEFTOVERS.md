...
| **V4A-4** | 规范铺底的第 2 步「写开发手册」与第 3 步的 agent 侧都还没起会话。**C 刀把第 3 步的确定性那半做了**(历史回填直接算 git 写文件,见 V4C-1),agent 那半仍未做 | 试点期 | 两份剧本(`merge-adjust` / `asset-audit`)已在 buddy 的资产目录里,缺的是「探测到之后自动起一次会话」那段编排。第 2 步 2026-08-20 已重新定性:不再是「把 buddy 的约定合进 AGENTS.md」,而是**把仓根 `AGENTS.md` 那份开发手册的「还没填」几节填成真的**(设计见 design/03 §2.3、剧本见 design/09 §2.3),目录名 `merge-adjust` 沿用 |
...
| **V4G-1** | 成熟仓里人自己写过仓根 `AGENTS.md` / `CLAUDE.md` 时,buddy **整份跳过、一个字不写**(对的),于是那份手册里就**没有** buddy 现探出来的构建命令与目录导览 —— 人写的那份缺什么就一直缺什么 | 试点期 | 这正是规范铺底第 2 步「写开发手册」要干的活(V4A-4):起一次会话去补进人写的那份,原文一字不删。那一步没做,所以今天只在铺底那张活的正文里如实列出跳过了哪几个路径、还没跑哪几步 |
