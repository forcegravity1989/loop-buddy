# 12 · 建法与交付记录:V4 是分七刀建出来的

> **30 秒导读**:这篇是 V4 从设计转开发的**交付记录**——七刀各自干了什么、代号怎么读、要看某件事的实况该去哪里。给两种人看:接着做 V4 的会话,以及被 `docs/code-schemes.md` 的代号索引指到这里的人。**现在还作数吗**:作数,但它只是一张索引——**任何「做到哪、怎么验」的细节都不在这里**,在下面 §3 指的那三个地方。原来那份逐步任务书(范围、默认假设、卡住预案、挂任务时粘贴的指令)在刀跑完之后已经没有读者,整块删掉了,要考古去 git 历史。
...
| **F · 提交并开 MR + 接入屏列真仓** | 补上第 4 站到第 5 站中间那一下:会话屏点一下,把这棵树里 agent 干出来的改动提交、推分支、开 MR、活进「评审中」。在这之前,干完的活没有任何办法变成别人看得见的东西。接入屏也在这一刀改成列真实的远端仓 | PR #108;设计见 [05-session-screen.md](05-session-screen.md) §2.4 |
| **G · 试点第一天** | buddy 铺进用户仓的件全收进 `.bw/`(仓根一个字不写);buddy 自带的技能**不再复制进用户仓**,改成开工时只给名字 + 路径的渐进式加载;通知屏只留「有 MR 等你合」一类 | PR #108 |

**A/B/C 在 PR #105,D 与 E 在 PR #106,F 与 G 在 PR #108 —— 七刀今天都已在 `main` 上。**
...
- **还没做完的**:`docs/LEFTOVERS.md` 的 V4A-N … V4G-N 七组,那是唯一的欠账登记表。
- **怎么验**:[10-e2e-acceptance.md](10-e2e-acceptance.md);开发命令另见仓根 `DEVELOPMENT.md`。

最大的一条偏差在这里点一句,免得只看本篇的人漏掉:**V4 另开了 `crates/bw-v4` 与 `crates/app-shell` 两个新 crate,没有在 `bw-app` / `bw-store` 上做加法**——库从二十张表缩到四张之后,旧内核的用例整片接不上。`bw-store` / `bw-app` / `ui` / `app-desktop` 至今一行没动,旧壳照常编译;`bw-core` / `bw-engine` 两个共用件被 V4 复用,偶尔为 V4 加函数(例如 2026-08-21 为代码仓级走势在 `bw-engine/src/github.rs` 加的按周查合入 PR),**只加不改既有行为**。这两个共用件里的 `bw-core` 将来要断掉,方向与已知的坑见 [01-architecture.md](01-architecture.md) §2.1。
