...
**拖拽**只能在待办池⇄待办之间,或列内部——跨列等于把活排进/移出当前选中的周,同列只调先后,不改任何状态;往其余四列拖,松手不发生任何事,只弹一句「状态流转用卡片上的按钮」。不想拖也行,卡片右键(或「…」菜单)有「排进本周」/「移出本周」两项,效果和拖拽一样(§2.3)。
...
预研结论:「能做,是小活」——Dioxus 0.7.9 拖放事件齐全,框架已在 JS 层全局 `preventDefault()` 了 `dragover`/`drop`(预研事实 2),不存在时序坑;跨列传卡片 id 不用 `DataTransfer`(桌面壳上是空操作),改用 `Signal<Option<IssueId>>`。**本篇全盘采纳**,落成产品规则:

- **可拖 = 待办池、待办两列的卡**,其余四列不带 `draggable`(和 `hifi/index.html:1664` 一致)。
- **可放 = 这两列自身**:跨列发 `ScheduleIssue{id,week_of}`(拖进待办 `week_of=Some(选中周)`,拖回待办池 `week_of=None`);同列发 `ReorderIssue{id,after}`(列内调先后)。
- **其余四列**拖动中显示不可放样式(灰化+`cursor:not-allowed`,参照 `hifi/index.html:1162-1170` 的 `kanban-col-noDrop`),松手不发命令,只弹 toast「状态流转用卡片上的按钮」(文案复用 `hifi/index.html:1142`)。
- **状态列绝不由拖拽改**:`ScheduleIssue`/`ReorderIssue` 只写 `week_of`/排序字段,不碰 `status`;`can_transition_to` 继续是唯一判分依据,拖拽路径物理碰不到它——这是对「Done 永不自动」「同一件活绝不重复记账」两条铁律在拖拽场景下的落实。
- **误触防护**:HTML5 原生拖放自带隐式阈值,不需要 BW 自己实现(预研 §7)。
- **键盘/右键替代**:卡片右键菜单(或「…」按钮)放「排进本周」/「移出本周」,发同一条 `ScheduleIssue`,补上拖放天生没有的键盘可达性。
...
底部「去会话 →」(未开工先发 `RunIssue` 再跳,已开工就是跳转+聚焦)、「蒸馏」复用 05 篇已定的 `DistillSkillFromIssue`。重开一张 `Done` 的活放在详情面板、要一次确认,不放卡片一键——密集卡片容易手滑。
...
**不做**:甘特图/里程碑实体(待拍-04 已定,版本就是里程碑);拖拽改状态(§2.3 已从命令设计上物理隔离);多版本线(待拍-04);「全部」视角下的跨列拖拽(§2.3 排除在首版范围外,`draggable` 只在选中某周模式下挂)。
...
- **`can_transition_to` 守卫读回**:往进行中/评审中/已完成/阻塞四列拖一次(模拟误拖),`sqlite3 <db> "SELECT status FROM issue WHERE id='<uuid>';"` 前后各查一次,应完全没变;再故意构造一次非法转移(比如给 `Backlog` 的活发 `TransitionIssue{Blocked}`),应被拒绝且 `status` 原地不动。
