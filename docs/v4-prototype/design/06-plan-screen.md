...
> **30 秒导读**:这篇管 V4 计划屏——一个界面里同时装下「这周要干什么」和「V3 那种六列看板」。给复核设计的用户、以后写代码的会话看。**状态:详细设计稿,待用户复核,尚未开工写代码。** 设计事实源是 [`../mvp-blueprint-draft.md`](../mvp-blueprint-draft.md)(下称母文档),与它冲突时以母文档为准;拖拽部分另有专门预研 [`../research/kanban-drag-dioxus.md`](../research/kanban-drag-dioxus.md)(下称预研),已确认技术可行,本篇负责把它落成产品规则。数据/schema 的技术权威来源是 [02-data-and-files.md](02-data-and-files.md)(下称 02 篇),本篇只应用它定的字段名/命令名,不另造一套。看不懂的词查 [`../../../CONTEXT.md`](../../../CONTEXT.md);代号查 [`../../code-schemes.md`](../../code-schemes.md)——本篇不新开代号系列,沿用「待拍-NN」。**2026-08-20 按用户第七轮盘点(库从 20 张表砍到 4 张:`project`/`issue`/`claude_conversation`/`app_meta`)整块重写了 §2.1 周列表算法与周头来源、§2.3 拖拽的落地写入、§2.4 发版本落库口径、§2.5 预览未合入的指标口径、§2.6 详情面板三个字段、§3 schema 与命令签名、§5 验收读回。**
...
**这套语义不依赖任何被砍掉的表(第七轮数据盘点核实)**:`can_transition_to` 是纯状态机函数,判断合法转移只看 `IssueStatus` 本身,不查 `workflow_run`/`agent`;上表里的「同项目无活在跑(串行锁)」「`is_running`」这两处忙态判断,来源是 `claude_conversation`(会话是否仍挂着活的进程/session,05 篇已把这张表定为"会话与活的对应关系正本")而不是已删除的 `workflow_run`——`workflow_run` 装的是"每次运行的成败与耗时",这条记账铁律在 V4 没有持久载体了(02 篇 §2.3,取代它的判据是远端 MR 合没合入),但"当前是否在跑"这个即时状态本就不需要历史记账表,`claude_conversation` 一张表就够。
...
**能不能点**:当前选中周存在至少一张 `kind='ops'` 的活处于 `InReview` 且 MR 未合入才能点开——覆盖运作活①(最常见)和③的「合并调整」/「历史回填」两步(它们的产出也动 `docs/plan/YYYY-Www.md`/`PROJECT.md` 草稿,待拍-21 原话「运作活的 MR 合入前」没限定死在①,本篇按更宽口径设计)。**没有单独的 `docs/plan/history.md` 文件**——02 篇第七轮盘点已明确取消:回填的历史周和本周文件同目录、同格式,只靠 front matter 的 `origin: backfill` 区分(02 篇 §2.5 样例),历史回填动的还是 `docs/plan/YYYY-Www.md` 本身,不是另一份历史文件。没有这样的活时开关灰,悬浮提示原因。
...
**指标现值要不要跟着预览切换,第七轮盘点后口径变了**:V4 已经没有 `observation` 表(02 篇 §2.1)——可重算的指标每次现场算(比如「本周合入活数」查 `git log --merges`),不可重算的手填读数直接写进 `docs/plan/YYYY-Www.md` 的「本周指标读数」段(02 篇 §2.5 样例),**和周目标、活清单同一份文件**。这意味着旧口径「观测数据不预览、指标卡现值保持读库真实值」已经不成立——库里根本没有一份独立于这份文件的「真实值」可以保持。新口径分两半:
...
// 母文档原文「切在研版本 = 改 project.current_version,不建活」写于
// current_version 还是 project 表一列的年代;02 篇第七轮已把这个字段移出
// 库、只存进 .bw/project.toml(一份仓文件)。这条命令签名因此存在没消解
// 的张力——继续"不建活"就意味着要绕开"凡写仓都是活"的规矩直接改仓文件,
// 还是并入常规轻量活+MR 流程,本篇不擅自选边,列入 §6 开放问题;下面的
