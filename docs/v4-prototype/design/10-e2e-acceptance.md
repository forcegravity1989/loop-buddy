...
**写代码的会话**:每完成一块回§2.1 找对应行,跑读回命令,把结果存进 `docs/v4-prototype/e2e/`(格式见 §5)。九篇第 5 节仍是正本,这篇只是串起来判断整体做完没有。
...
不是单独检查,是"§2.1 每行跑完后存进 `docs/v4-prototype/e2e/`"本身(格式见§5)。判据:01-09 每篇≥1 条记录;六入口各≥1 条含 `[BW_OPEN]` 的深链证据。
...
这篇不改 `crates/`,也不产生 `docs/v4-prototype/e2e/` 目录(现在建空目录没意义)。开工顺序:01-09 篇实现完成后回§2.1 找对应行、跑读回、按§5 存证据;`real_demo_v4.rs` 按§3 骨架实现,建议在04-09篇功能大致落地后再写,写完后§2.2 表格就是实现清单与验收清单合一。
