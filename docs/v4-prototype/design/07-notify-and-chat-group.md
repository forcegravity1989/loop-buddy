...
- `.bw/project.toml`/`.bw/metrics.toml` 的完整 schema 与迁移——见 [02-data-and-files.md](02-data-and-files.md);本篇只引用已经在母文档 §6 与 `standard-module-draft.md` 第 1 类里定形状的 `[chat]` 段与 `chat_outbox`,不重新定义。
...
**对应母文档**:[`mvp-blueprint-draft.md`](../mvp-blueprint-draft.md) §1 愿景③与反命题段、§2.6 触发与介入全景表(通知列、运作活①群摘要、评审行「合入并完成」)、第 5 站(验证闭环)、§3.5 易用性表、§5 信息架构(通知行 / 配置行④ / 设置行)、§6 数据模型增量(`project.chat_*`、`chat_outbox`)、§7 建法(项目群适配工厂段)、§8 验收第 8 条、待拍-13/26/28。
...
**配项目群**:总览名片区「项目群:未配 · 配置」,点开填提供方(内部选 WeLink,填群号)和「哪些事同步到群」(默认评审中/已合入/发版三个都勾)——走的是与「编辑项目信息」相同的轻量活 + MR 流程(08 篇),合入后名片变「项目群:后端小组 · 通知同步中」,往后通知行右边多一个「已发到群 ✓」小字。
...
| 已同步到项目群 | `chat_outbox` 里状态变成「成功」的行——事件流这一条和通知行上的「已发到群 ✓」小字是同一份数据的两处展示,不是两套记账 |

### 2.3 未读怎么算——不建 `notify_seen` 表
...
事件流段是纯日志,同样不需要「已读/未读」的业务语义——它不驱动任何待办。但为了让人一眼看出「上次看过之后又发生了什么」,本篇提出一个很轻的辅助机制:一张极小的表 `notify_seen(project_id, last_seen_at)`,记「这个项目的事件流我看到了哪个时间点」,每次打开通知屏更新一次。事件流里晚于 `last_seen_at` 的条目在界面上加粗/带一个小圆点,仅此而已——**这个时间戳完全不参与徽章计数**,纯视觉辅助,丢了、清零都不影响任何业务状态,符合「谨慎数据库」的三层信息架构(本机库只做记账与推导,这张表体量极小、允许丢)。这是 `MarkNotifySeen` 命令存在的唯一理由,见第 3 节。
...
① `settled_at` 已有值或 `status == Done` → 幂等短路,提示「已完成,无需重复合入」,不重复调用远端、不重复记账。② `status == InReview` 且 `pr_number ≠ 0` 才继续,否则拒绝并提示改用「点完成」。③ 项目未挂远端仓(`remote_path` 为空)→ 拒绝。④ 调用 `remote.merge_mr(pr_number)`(经 `Remote::for_project` 路由到 github/codehub)——失败则「merge PR/MR #N 失败,活留在评审中:<错误原文>」,状态不变、可重试。⑤ 成功后 `Box::pin` dispatch `Command::TransitionIssue { status: Done }`,复用既有「同一件事绝不记两次」的记账机制(`settled_at` 写入、战绩记到 workflow/技能、artifact 回流、工作区与指标同步),不新增第二套记账路径。⑥ 项目配了群且 `notify` 勾了 `"merged"` → `SyncNotifyToChat` 异步发一条,失败不回滚已发生的合入与记账(见第 4 节)。
...
- **本机设置**:「聊天工具登录」(设置屏,与 codehub/GitHub 登录并列)。登录态(WeLink 的 access_token 或别的凭证)不进仓、不进库——这条纪律直接抄 `Remote` 现有的两个先例:GitHub 靠 `gh auth login` 自己的本机配置,CodeHub 靠 `codehub-cli auth login` 写 OS keyring(预研 §1 已核实,`crates/bw-engine/src/github.rs`/`codehub.rs` 模块文档原话)。群号是项目的事(进仓),登录是机器的事(留本机)。
...
- **去重与重发**:`chat_outbox` 记 `(issue_id, event_type)`,发前查有没有成功行,有则跳过、绝不重发;发送先写「尝试中」,成功回填、失败保留待 `tick_scheduler` 重试。去重键粒度留 02 篇详设(预研开放问题 5)。
- **失败如实**:发群失败不回滚已发生的合入/完成/发版,但通知行要看到「发群失败 · 重试」,不悄悄吞掉。
...
- **要实现的函数、输入输出、错误约定**:见 2.6——`send` 必须实现,`fetch_history` 尽力(做不到就稳定返回 `HistoryUnsupported`,不抛异常不 panic),`probe` 可选但建议做(配置页「测一下」体验好很多);`Auth`/`Network` 把 WeLink SDK/HTTP 的原始错误文本原样带上,**绝不吞掉错误伪装成功**(全仓「读回为证」纪律的延伸)。
...
- **不用真群怎么自测**:先对着 `mock`(2.6 表格,本篇范围内落地)把 buddy 这边的调用链(通知同步、去重、运作活①摘要生成)跑通,再换真实 WeLink 凭证验证 `send`/`fetch_history` 本身;`none` 用来验证「没配群时别的流程不会崩」。
...
| `SyncNotifyToChat { issue_id, event_type }` | 按 2.7 的模板拼一条消息、经工厂拿到的 `ChatGroup` 发出去,记一行 `chat_outbox` |
...
**表**(引用母文档 §6 已定的增量,不重复定义 schema 细节,详设见 02 篇):

- `project`:`chat_provider` + `chat_group_id`(`[chat]` 段的库存副本)。
- `chat_outbox`(新,小):事件类型、活、时间、成败——通知同步的去重与重试凭这张表。
- `notify_seen`(新,极小,本篇提出,需 02 篇确认):`project_id`、`last_seen_at`——仅供事件流视觉状态使用,丢了不影响任何业务判定。
...
**事件**:沿用现有 `Event` 总线的做法(如 `Event::IssuesChanged`/`Event::ConnectorSynced`)新增对应变体即可,不需要新总线;`chat_outbox` 成功/失败与「已同步到项目群」事件流条目是同一条数据在两处的展示,不需要两套事件。
...
| `SyncNotifyToChat` 调用 `send` 返回 `Auth`/`Network` | 通知行「发群失败 · 重试」,不影响已经发生的合入/完成/发版 |
| `SyncNotifyToChat` 调用 `send` 返回 `NotConfigured` | 不算失败,直接跳过,不产生 `chat_outbox` 行(项目没配群这件事本身是诚实状态) |
...
| WeLink 未按预研骨架实现某个函数(如 `probe` 没写) | trait 默认实现兜底返回 `NotConfigured`,配置页「测一下」按钮显示「该提供方未提供测活能力」,不崩 |

---

## 5 · 验收与读回

1. **`chat_outbox` 去重读回**(`mock` 提供方):演示项目提供方设 `mock`,依次触发一次评审中、一次合入、一次发版,`sqlite3 <db> "SELECT issue_id, event_type, status FROM chat_outbox WHERE project_id = <id> ORDER BY id"` 正好三行、状态都成功;对同一 `(issue_id, event_type)` 重复触发评审中事件,行数仍是三行。
...
5. **`none` 提供方不崩**:项目 `chat_provider = "none"`,触发一次合入与一次运作活①,`chat_outbox` 无该项目新行,运作活①正常走完,验证「没配群时别的流程不会崩」。
...
3. **`chat_outbox` 去重键的粒度**(继承预研开放问题 5):`(issue_id, event_type)` 够不够,还是要考虑「同一 issue 先后开过两个 MR」这类边界情况多加一维(如 MR 号),留给 02 篇数据模型详设时定。
4. **通知失败重试的具体节律**(继承预研开放问题 4):靠 `tick_scheduler` 哪个 tick、退避几次、多久放弃改成「需要人手动重试」,留给实现时定。
