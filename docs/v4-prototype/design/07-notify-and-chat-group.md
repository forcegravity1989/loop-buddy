...
- **怎么喂**:作为第 4 层项目知识旁的参考件,和 `docs/plan/`、`.bw/metrics.toml`、codegraph 索引一起注入;注入时机由 [09-ops-workflows.md](09-ops-workflows.md) 引用本篇。**`fetch_history`/群摘要只喂给运作活①这一张活**(确认:握手清单 第 4 条「回填不主动喂群历史」)——运作活②「资产盘点」不论 `mode=weekly` 还是 `mode=first`(老项目历史回填)都不调用 `fetch_history`,03/09 两篇已按此口径写,本篇不重复。
