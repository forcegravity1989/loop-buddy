...
> **30 秒导读**:这篇管运作活③「规范铺底」——项目接入时自动出现的一次性活——具体怎么跑:第 1 步 buddy 自己写模板核心件、第 2 步(agent)把仓根 `AGENTS.md` 那份开发手册写成真的、第 3 步(有历史的仓才有)把老项目自己的历史记录回填成 buddy 认得的文件;外加铺完之后的日常动作:对账、升级、`standard/` 正本怎么让同事贡献。**历史回填不是这里养的一个独立技能**,它是运作活②「资产盘点」的首次模式(同一个 `asset-audit` 包,`mode=first` 全量、`mode=weekly` 增量),剧本归 [09-ops-workflows.md](09-ops-workflows.md) 管,本篇只管「探测到历史该不该跑、原料与产物长什么样、怎么防伪」。**现在还作数吗**:作数,而且已经落地——V4 的内核 `crates/bw-v4` 与新壳 `crates/app-shell` 都在 `main` 上,第 3 节「工程对照」写的是真代码的结构。还没做完的部分只认 [`../../LEFTOVERS.md`](../../LEFTOVERS.md) 的 V4A–V4E 五组。 给三种人看:接着做 V4 的会话、往 `standard/` 提 PR 的同事、要核对铺底行为的人。看不懂的词查 [`../../../CONTEXT.md`](../../../CONTEXT.md);代号查 [`../../code-schemes.md`](../../code-schemes.md)。

---

## 0 · 这篇管什么、不管什么

**管**:①探测——两卡填完后怎么判断运作活③跑几步(空仓例外这一项 2026-08-20 已取消,见 §2.1);②第 1 步「写核心件」全流程;③第 2 步「写开发手册」agent 任务清单(这一步 2026-08-20 已重新定性,见 §2.3);④第 3 步「历史回填」全设计——原料分给脚本/agent/人、四份仓文件产出物字段(+ 一行库缓存例外)、防伪规则、幂等、限时/抽样、缺的函数清单;⑤三步合成一个 MR、人怎么评审;⑥对账与升级流程;⑦`standard/` 正本结构、版本号、同事怎么贡献;⑧本篇命令/事件名字。

**不管**:`standard/` 放哪、怎么进二进制、版本常量叫什么——[01-architecture.md](01-architecture.md) §2.9 已定,本篇直接引用。`.bw/*.toml` 与仓文件**格式**——[02-data-and-files.md](02-data-and-files.md) 已给,本篇只补留白处(`.bw/managed.toml` 指纹算法、回填历史周文件里"累计贡献者"字段落在哪一份文件)。「写开发手册」「历史回填」两个技能的 **SKILL.md 正文剧本**——[09-ops-workflows.md](09-ops-workflows.md)管,本篇只给任务清单。计划屏周列表 / 总览发版记录怎么把回填的历史周、历史版本渲染出来(不另开专门块,只带徽记)——[06-plan-screen.md](06-plan-screen.md)/[08-overview-derivation.md](08-overview-derivation.md) 已给,本篇只保证产出的形状(同格式的历史周文件、`.bw/releases.md` 历史版本行)能被它们直接消费。开工工具注册、workflow 识别——[04-tools-and-workflows.md](04-tools-and-workflows.md)管。
...
**三步里今天只跑通了第 1 步。** 已落地的是**第 1 步「写核心件」**(`crates/bw-v4/src/standard/bootstrap.rs::write_core_files`,§2.2 描述的落盘规则与其对应)。第 2 步「写开发手册」与第 3 步「历史回填」的 agent 那一半都要起一次真实会话去读文本、写产出,**还没做**——`run_standard_bootstrap`(`crates/bw-v4/src/app/bootstrap.rs`)探测到仓需要跑这两步时,只把「还没跑的步骤」如实写进这张活的说明段落,不假装跑过。下面 §2.2 是已经跑通的部分;§2.3/§2.4 是设计,阅读时按此对待。
...
| 触发第 2 步 | README/CLAUDE.md/AGENTS.md 三个路径存在性检查(有 = 第 1 步跳过没写,得靠会话补) | 新增,放进 2.2 节要建的 `standard_bootstrap.rs`,不需要新模块 |
...
2. 按清单逐类从 `standard_assets` 取模板渲染到目标路径:
   ...
   - 第 2 类 → 仓根 `AGENTS.md`(模板渲染,构建命令与目录列表现探;**仓里已经有就整份跳过**)+ 仓根 `CLAUDE.md`(仅一行 `@AGENTS.md`,待拍-15);
...
### 2.3 第 2 步 · 写开发手册(agent)—— 2026-08-20 重新定性,整节重写

**原来这一步叫「合并调整」,做的是「把 buddy 的固定章节合并进已有 AGENTS.md」。
那是反的,已经推翻。**

推翻它的理由:那些「固定章节」——先读什么 / 活怎么做 / 指标怎么碰 / 禁止事项——
**全是 buddy 自己的运作规矩**,而它们今天已经由 `agent_system_prompt` 每场会话
新鲜注入了。再往每个用户仓里写一份,就是第二个会漂移的副本,而「规范版本 + 指纹
对账」那整套机制,存在的唯一理由就是追这个副本漂没漂。副本不写,机制就不用追。

反过来,用户真正想从 buddy 这儿得到的是:**被 buddy 纳管的项目,对 AI coding
更友好** —— 仓里有一份说得清「怎么建、怎么跑、怎么测、目录里什么在哪、这个项目
和别处不一样在哪」的手册。那才是仓里该有的东西,而原来的模板里它是一节
「(待填)」的空壳。
...
| 内容 | 住哪 | 谁维护 |
|---|---|---|
| buddy 的铁律(完成由人点、指标只能真采、合并永远是人)| 只在系统提示词里 | buddy,随版本走 |
| 「本项目由 buddy 管理,`.bw/` 下是正本」 | 仓根 `AGENTS.md` 里一小段,带 `bw:managed` 标记 | buddy,升级时整段替换 |
| 怎么建 / 跑 / 测、目录导览、项目特有的规矩 | 仓根 `AGENTS.md` 主体 | buddy 搭初稿,之后归项目 |
| 项目说明(做什么、对标谁、北极星)| `.bw/PROJECT.md` | 接入两卡 + 改名片 |

**落点在仓根,不在 `.bw/`**:`AGENTS.md` / `CLAUDE.md` 是 Claude Code、Cursor、
Codex 共同约定的位置,塞进 `.bw/` 谁都读不到。它们不是 buddy 的资产,是项目的
资产 —— 所以不违反「buddy 的东西都收进 `.bw/`」那条。

**第 1 步已经能填的**(不起 agent,读文件就能确定,`standard::detect`):
构建系统与命令(`Cargo.toml` / `package.json` 的 scripts / `pyproject.toml` /
`go.mod` / `Makefile`)、CI 配置在不在、顶层目录列表。**认不出来就写一句明说
「还没填」,绝不猜一条命令写上去** —— 写错一条构建命令比留空更糟,人照着跑
一次失败才发现是编的。

**第 2 步(agent,还没实现)要做的**:读 README、构建文件、CI 配置、逐个顶层
目录,把「还没填」那几节填成真的;每条命令真跑过一次才写进去;MR 说明里一节
一行交代内容是从哪读来的。剧本 `merge-adjust`(名字沿用,内容已整篇重写)在
`standard/06-defaults/ops/standard-bootstrap-agent/merge-adjust/SKILL.md`。

**仓根本来就有人写的 `AGENTS.md` / `CLAUDE.md` / `CONTRIBUTING.md` 时**:第 1 步
按「人手改过的不覆盖」跳过,一个字不写,并把跳过的路径写进那张活的正文。第 2 步
的活因此变成**补进人写的那份**:原有内容一字不删,缺的几节补上,`bw:managed`
那段加在最前面。冲突交给会话去谈、交给 MR 去评审,**不交给模板去覆盖**。

**产出**:仓根 `AGENTS.md` / `CLAUDE.md`,提交在同一条分支上,不单独开 MR。
...
**还没做的**:第 2/3 步(写开发手册、历史回填)要真起 agent 会话,产出「手册里哪几节填成了真的、内容从哪读来的」「回填了哪些数字」这类内容供人评审——这两步还没实现,所以今天的 MR 里只有第 1 步写的核心件,「三步拼成一份 MR 说明」这件事还不存在。
...
仓根 `standard/` **今天真有**下面这些目录 / 文件(八大类不是全都铺开了):`06-defaults/ops/` 已经建了,里面是三份自建运作 workflow 的正本;`CHANGELOG.md`、`07-cadence/`、`pond/` 仍然没建。
...
│       └── standard-bootstrap-agent/merge-adjust/SKILL.md   # 运作活③第 2 步「写开发手册」(目录名沿用旧名,内容已整篇重写)
└── 08-meta/
    └── standard.toml.tmpl
