...
| 运作活③规范铺底 v5.0 | 已完成 08-17 | 含写开发手册,一次性,不会再出现在下一周 |
...
| 仓根 `AGENTS.md`(+ `CLAUDE.md` 一行导入) | **这个项目的开发手册**(怎么建/跑/测、目录导览、项目特有的规矩);buddy 自己的铁律不在这儿,在系统提示词里 | 铺底第 1 步写模板 + 第 2 步 agent 填真 | 项目自己的 AI 工具读(Claude Code / Cursor / Codex) |
