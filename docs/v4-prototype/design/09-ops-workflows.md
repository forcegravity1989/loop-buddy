...
项目第一次接入(或老项目重铺规范)时人只填两张卡片——若仓有历史,同一张活再多一步——起**运作活②「资产盘点」workflow 的首次模式**,把能找到的历史整理成带「回填」标记的文档(用户定性:老项目历史回填就是资产盘点第一次跑,以后每周跑的是同一个 workflow 的增量模式),一个 MR 全带上,人评审、合入、点完成——总览多出「历史运作(回填)」块,老项目瞬间有了和新项目一样的骨架。
...
**触发与判据**:人触发,两个入口——总览横幅「开始本周」(最顺手),或计划/会话屏手动建一张 workflow 选「更新指标与周计划」的运作活(同一条命令,横幅只是快捷方式)。命令 `StartWeekPlanning`(见 §3)。

判据是**两条,分管两件事**(2026-08-21 按实况改写):

- **横幅出不出**:看当前 ISO 周的 `.bw/plan/YYYY-Www.md` **文件在不在**。文件还没有、但这周的运作活①已经在路上时,横幅换成另一态——不再给「开始本周」按钮(再点一次只会被挡),而是给一条去会话屏的路。
- **会不会建出第二张活**:**按标题查**(`ops1_title(week)`),不按「这周有没有活」查。按周查的话,人把这张卡拖回待办池就查不到了,而建活本身又是按标题幂等的,于是会拿回同一张活重新开工——那张活可能已经在「评审中」。查到同名活且不是「已完成」,就直接如实回一句「这周的运作活①已经在跑,状态是 X」,不再建、也不再开工。
...
> - **第二步·更新指标**:调用子技能 `metrics-refresh`(合并自 north-star-discovery/metrics-binding,见 2.4)——补缺的引领/滞后、给绑不上的找最便宜采集路径、跑一次采集;绝不伪造观测、绝不为点亮改指标定义。**采集路径按 [14 篇](14-metrics-collection.md):只有「脚本」和「手填」两种,脚本落在 `.bw/scripts/`,没有 `.bw/connectors.toml` 这一层**;每撞到一条手填指标都要认真问一次「这个数能不能写成脚本」(14 篇 §2.7)。改动先不提交,和第四步合成一次提交。
...
**产出**:仓——`.bw/metrics.toml`/`.bw/scripts/*`(若新增采集脚本,[14 篇](14-metrics-collection.md) §2.4;原来这里写的 `.bw/connectors.toml` 已随「连接器」这个中间概念一起取消)/`docs/metrics.md`/新建 `.bw/plan/YYYY-Www.md`(周目标 + 活清单 + 本周指标读数**快照**——读数正本已按 14 篇 §2.5 移到 `.bw/metrics/readings.jsonl`,周文件里那一段只是抄一份给不装 buddy 的人看);库——运作活①本身一行(`kind='ops'`/`origin='human'`/`workflow='更新指标与周计划'`)+ 每张确认业务活各一行(`origin='agent_split'`,待拍-08,`week_of`/`version`/`tool`/`metric_key` 等 8 个缓存列随建活一起写入,02 篇 §2.2)——**没有 `issue_metric`/`week_plan` 这类关联表或索引表**(已取消):指标挂哪由 `issue.metric_key` 单列表达,周列表靠扫 `.bw/plan/` 目录得到(02 篇 §2.1/§2.6)。MR——一个,标题「周计划 2026-Www」,挂在运作活①上。

**周计划文件谁写(2026-08-21 按实况改写,真跑与替身是两条路)**:

- **真跑**(桌面壳开着内嵌终端 + 项目有真实工作区):点「开始本周」这一下**一个文件都不写**。周计划文件由运作活①的 agent 会话在这张活自己的 worktree 里产出、走 MR、人合入后才落进主检出——仓是正本,合入才算数。原来这里会先往主检出写一份骨架,那会留下一个没进版本控制的同名文件:MR 合入后 `git pull` 被它顶住,而总览一直显示骨架里的【mock】目标。
- **替身**(headless 指挥器,或项目没配工作区):没有真会话可以产文件,骨架 + 【mock】草稿就是这条路的全部产出,**处处自我标注**,不冒充真实对话的结论。
...
**注入清单**:第 0 层系统提示词、第 1 层仓根 `AGENTS.md`(**人自己写过才有;buddy 不铺这份**)、第 2 层本活技能 `asset-audit`(含 `mode` 参数)与它带的子技能 `project-handbook`、第 3 层规范第 3 类「目录与知识结构」件+第 6 类「默认件与鱼塘」件(`mode=first` 额外注入 03 篇 §2.4 描述的本机 evidence 文件,不进仓不进库,同「上周群消息摘要」待遇)、第 4 层 `.bw/PROJECT.md`+本周 `.bw/plan/`(盘点报告要追加进去)+codegraph 索引。**②排除**项目群摘要——不管哪种模式都不读群历史,内部盘点/历史回填都不需要「群里聊了什么」(母文档 §2.6 用户四问第 2 条已定,03 篇 §4 同步排除)。
...
**`mode=first` 里多一个提议:给这个项目写开发手册**(2026-08-20 从运作活③挪过来的,
03 篇 §2.3 说了为什么)。子技能 `project-handbook`,住在
`standard/06-defaults/ops/asset-audit/skills/project-handbook/SKILL.md`。要点三条:

- **先读**:仓根有没有人写的 `AGENTS.md` / `CLAUDE.md` / `CONTRIBUTING.md`、README、
  构建文件、CI 配置(**空的 `.github/workflows/` 不算**)、逐个顶层目录。
- **再问**:把读出来的摊给人看,说清楚要动哪个文件,**人点头才写**。人说不要、
  或者说不确定,都当不要 —— 沉默不是同意;把「提过、人不要」写进回执,下次不再问。
- **才写**:每条命令真跑过一次才写进去;人写过的原文一字不删,只补缺的几节;
  buddy 自己的规矩一个字都不往里写(那些每场会话都注入)。

**为什么归这里而不是接项目那一刻**:仓根那两份是**项目自己的**文件,写它等于
「建议改造你的项目」——要先读懂这个仓、先问过人,这两件事只有一次真 agent 会话
做得到。
...
**时长与轮次(估计)**:`mode=weekly` 约 10-25 分钟,人机对话通常 0 轮;`mode=first` 差异大,见 03 篇的估计段(03 篇/legacy-backfill.md 已提示大仓可能明显更长)。

---

### 2.3 运作活③「规范铺底」——**没有需要 agent 的步骤了**(2026-08-20 傍晚整节重写)

铺底就一步:buddy 自己把核心件写进 `.bw/`,提交、开 MR、停在评审中。
**不起 agent,不碰仓根。** 判据、探测、落盘规则全在
[03-standard-and-backfill.md](03-standard-and-backfill.md),本篇没有可写的剧本。

原来这里有一节「合并调整」→ 改名「写开发手册」→ 现在**整个取消**。理由与两次
推翻的经过在 03 篇 §2.3,一句话:给别人的仓写开发手册,性质是「建议改造你的
项目」,得先读懂那个仓、先问过人 —— 那是一次真 agent 会话干的事,不是接项目
那一刻能干的。它已经挪进 §2.2 资产盘点的首次模式,技能名 `project-handbook`。

铺底探测到仓有历史时,仍然会触发一次 `asset-audit(mode=first)`(§2.2),
两次会话的改动合成同一个 MR。这条没变。
...
└── asset-audit/
    ├── SKILL.md                  # 入口②——读 mode 参数分支:weekly(默认,每周)/ first(老项目历史回填,
    │                             #   原挂在③下的「历史回填」子技能并进这里,不再单独存在)
    └── skills/project-handbook/SKILL.md  # 子技能,first 模式里的一个提议:给这个项目写开发手册
                                  #   (2026-08-20 从③挪过来的;③已经没有 agent 步骤了)

入口③「规范铺底」**没有目录** —— 它不起 agent,没有剧本可放。
```

**不进项目仓**:这两份(含子技能 `metrics-refresh`、`project-handbook`)和九篇方法论技能一起编在 buddy 二进制里,开工前展开到 buddy 自己的资产目录,系统提示词只给名字+一句话+路径。原来定的「铺底时整棵目录物化进项目仓 `.claude/skills/`」已于 2026-08-20 推翻,见 [04 篇](04-tools-and-workflows.md) §2.7。
...
- **原样沿用**:north-star-discovery 的三段拆解、虚荣指标黑名单、自动化免疫检验、北极星判据打分 6 项、反指标机制、BDFE 输入结构、NSM↔商业 KPI 校验;metrics-binding 的硬性约束(绝不伪造、绝不为点亮改定义)、搭采集装置的流程。
- **不沿用**:metrics-binding 里「按 `collect.kind` 分支的诊断表」——五种采集方式已被 [14 篇](14-metrics-collection.md) §2.2 收成两种(脚本 / 手填),分支表跟着作废。`metrics-refresh` 要按 14 篇 §2.1 的判据引导人判每条指标「历史还能不能重新算出来」,并按 §2.7 把手填的往脚本上推。
...
// ③只有一步:buddy 直接把核心件写进 .bw/,仓根一个字不写(2.3)。唯一会追加
// agent 会话的分支是「仓有历史」——起运作活②同一个 asset-audit workflow、传
// mode=first,不是③的子技能。函数名与探测结构体照抄 03 篇 §3.1/§3.2。
pub(crate) async fn run_standard_bootstrap(&mut self, p: ProjectId) -> Result<(), AppError> {
    ...
    self.write_standard_core_files(p, &probe).await?; // 第1步,同步,不起 agent
    {
        ...
        // 铺底自己没有 agent 步骤了(2.3)。只有「仓有历史」这一条会另起一次
        // asset-audit(mode=first) 会话,在同一分支继续提交;否则直接开 PR。
        // 给这个项目写开发手册是那次盘点会话里的一个**提议**,要先问过人,
        // 不由这里编排。
        let need_agent_step = probe.has_history;
        if need_agent_step {
...
3. ~~`north-star-discovery`/`metrics-binding` 旧文件的迁移时机~~ **如实更新(2026-08-21 复核)**:内容已经合并进 `standard/06-defaults/ops/week-planning/skills/metrics-refresh/SKILL.md`,但**旧的两份文件还在** `docs/skills/` 里,也仍然被 `bw_core::bw_library::bw_standard_skill_docs()` 编进二进制、仍然出现在技能清单里。两份并存,违反「不为向后兼容留旧路径」。**一处更正**:原文还说它们「仍然随铺底复制进项目仓」,那已经不成立——2026-08-20 起 buddy 自带的技能一律不复制进用户仓(§2.4 末段、[04 篇](04-tools-and-workflows.md) §2.7)。删除动作记在 [`../../LEFTOVERS.md`](../../LEFTOVERS.md),没做就是没做,不在这里写成"已迁移"。
