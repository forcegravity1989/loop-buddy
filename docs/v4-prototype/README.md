...
> **30 秒导读**：V4 的规划入口。2026-08-17 进入特性规划期；**2026-08-18 全貌草案经三轮 grilling 收口，转为设计事实源**；2026-08-19 内部专家评审后草案进第四轮（映射三列 / 看板拖拽只排期 / 项目群 / 老项目回填 / 易用性守则），高保真升 v4，**详细设计开写（`design/`）**（见下表）。不要往这里塞 V3 的 bug 修法。当前出包与 V3 修 bug 见 [`../releases.md`](../releases.md)；还没干的活见 [`../LEFTOVERS.md`](../LEFTOVERS.md)；文档写哪见 [`../doc-boundaries.md`](../doc-boundaries.md)。
...
| [mvp-blueprint-draft.md](mvp-blueprint-draft.md) | 2026-08-18 从口述 + V1-V3 实践 + 代码事实推出的 V4 MVP 全貌草案（七站旅程 / 指标·周目标·活关系 / §2.6 触发与介入全景表 / 信息架构 / 数据 / 建法 / 验收）；第三轮 22 条全部已定；**第四轮（2026-08-19 专家评审）加 待拍-24~28 与 §3.5 易用性审视** | **作数**（V4 设计事实源）；同名 .html 是第三轮的审阅版（顶部有第四轮说明），内容以 .md 为准 |
...
| [hifi/](hifi/) | 高保真可点击原型 v4：[README.md](hifi/README.md) 是简报（逐屏内容 + 样例数据出处，样例 = buddy 自己的仓 2026-W34；§0 三张「反馈 → 决定」表），[index.html](hifi/index.html) 是单文件原型（浏览器直接开；顶部常驻「演示数据」横幅，✱ = 假设值） | 用户拍板顺序：**先高保真 → 反馈 → 再详细设计**；v4 = 专家评审反馈已折入 |
| [design/](design/) | **详细设计稿**（第一版，2026-08-19 夜）：[README](design/README.md) 是目录 + 七节模板；01 架构与模块管控 · 02 数据与文件格式 · 03 规范铺底与老项目回填 · 04 开工工具与 workflow · 05 会话屏 · 06 计划屏 · 07 通知与项目群 · 08 总览推导 · 09 运作活剧本 · 10 验收与 E2E · 11 知识库；`00-handshake.md` 是给用户的二次握手清单，`REVIEW-2026-08-19.md` 是交叉复核与开放问题汇总 | **待用户复核**；不改 crates/，复核后开工 |
