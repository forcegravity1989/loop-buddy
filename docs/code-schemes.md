...
| **V4-A…G**(刀号) | `docs/archive/v4-prototype/design/12-build-plan.md` §1(已归档) | V4 开发的七刀:A 骨架+数据+主环 / B 运作活+会话屏 / C 回填+项目群+包 / D 九屏照高保真重排 / E 每张活一棵 worktree+铺底提 MR / F 提交并开 MR + 接入屏列真仓 / G 试点第一天:铺底收进 `.bw/`、技能不进用户仓、通知只留 MR。A 刀内再分 A1 数据层 … A7 收尾,B 刀内再分 B1 运作剧本 … B8 收尾(逐步任务书已随刀跑完删除,考古去 git 历史)。**与 plan/06 的 A0–A5、plan/18 的 A1/A2/A4 无关** | 七刀全部已合入 main:A/B/C 在 PR #105,D/E 在 PR #106,F/G 在 PR #108 |
...
| **V4D-1…** | `docs/LEFTOVERS.md` | V4 D 刀(九屏照高保真重排)留下的遗留条目(远端来源的指标、要先改规范格式的字段、符号影响面与依赖图之类) | 开着 |
| **V4E-1…** | `docs/LEFTOVERS.md` | V4 E 刀(每张活一棵 worktree + 规范铺底提 MR)留下的遗留条目(会话跑完不自动开 MR、脏 worktree 没人收、分支基线之类) | 开着 |
| **V4F-1…** | `docs/LEFTOVERS.md` | V4 F 刀(提交并开 MR + 试点第一天的接入/设置修补)留下的遗留条目(只列自己账号下的仓、不替你 clone、库路径仍只读) | 开着 |
| **V4G-1…** | `docs/LEFTOVERS.md` | V4 G 刀(试点第一天下半场:铺底的件全收进 `.bw/`、buddy 自带技能不进用户仓改走渐进式加载、通知只留「有 MR 等你合」一类)留下的遗留条目 | 开着 |
| **试点-1 … 试点-N** | `docs/LEFTOVERS.md`「当前开着」表 | 2026-08-21「试点第 1-3 站 + V4 文档整理」那次复核出来的欠账:**不属于任何一刀**,所以不进 V4A–V4G 任何一组。内容偏结构与依赖(断 `bw-core`、技能两个正本目录、连接器孤儿显示、替身写假指标文件、`bw-engine` 死链与业务语义泄漏、指标采集整套没接、`enabled` 死字段)。**中文前缀,不与任何字母系列撞车**,同 `减负-N` 的做法 | 开着 |
| **V4-待拍-01–32** | `docs/archive/v4-prototype/mvp-blueprint-draft.md` §11(已归档) | V4 MVP 全貌草案的决策项编号(两轮用户反馈 22 条 + 高保真期新增 23 + 2026-08-19 内部专家评审新增 24–28 + 2026-08-20 用户三轮回复新增 29–32:信息住哪盘点后库只剩四张表 / V4 不兼容老库 / WeLink 登录不归 buddy 管 / 预置技能包随 buddy 出厂并复制进项目仓);**2026-08-21 起,V4 设计正本 `docs/v4-prototype/design.md` 正文不再引用这套编号**——它是设计期的过程记录,考古用 | 已定,原文归档 |
| **V4-规范大类 1–8** | `docs/archive/v4-prototype/standard-module-draft.md` §2(已归档) | 规范铺底模块的八个大类,**故意用数字不用字母**(A-H 会与本表 A/B/G 撞车);正文里写作「第 N 类」+ 名称;规范今天真有哪些目录以仓根 `standard/` 为准 | 已定,原文归档 |
| **V4-加载五层** | `docs/archive/v4-prototype/mvp-blueprint-draft.md` §2.6(已归档) | agent 会话里规范手册的渐进加载层次,**故意不编字母**(与 plan/03 的 L0–L6 派生链撞车),正文写「第 0–4 层」;现役表述在 `docs/v4-prototype/design.md` 第 2 章 | 已定 |
