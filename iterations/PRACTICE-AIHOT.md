...
## 5. plan/11 续篇(2026-07-21,用户五点新指令 + 一次拷问)

见 `plan/11-boards-process-cards-and-real-aihot-loop.md`(fable 出计划,sonnet5 执行)。
五条工作线 L1–L5,只记这里跟 aihot 直接相关的一条(L5);其余四条(项目组件
详情卡、看板/过程件分栏、工作流流程图、业界卡片信息架构)是 BW 通用 UI,
落在 `crates/app-desktop`/`crates/ui`,不专属 aihot,详情见 plan/11 正文与
对应 commit。

**L5 · aihot 运行态/开发态双环拆分——真实发现一个配置错误,真实修正**:
上一夜(§4 K4)注册的 cron「aihot 每日日报生成」= Daily + `CreateIssue`@Build——
对一个已经建成、每天在真实产出的产品,这等于"每天自动建一件开发任务",
把「运行态」(产品自己每天产出)和「开发态」(改进产品本身)混成一件事。
真实修法(`cmd_cron`,`practice_aihot.rs`):

1. 老任务如果还在且没暂停,真实 `SetCronStatus(Paused)` 暂停它——**留痕,
   不静默删除**;sqlite 读回 `status=paused`,名字和错误配置原样留着,
   审计能看见"这里错过一次"。
2. 注册新任务「aihot 治理复盘 · 按需开发」= Weekly + `CreateIssue`@Optimize
   (阶段从 Build 改 Optimize——持续改进已建成产品,不是从零构建)。
3. **诚实局限,不假装做到**:真正该做的是"命中率跌破阈值才触发",但
   `bw_core::model::cron_due` 只按经过时间判断(`now - last >= 周期`),
   没有读指标值的能力——给调度器加度量条件是比这条 cron 大得多的引擎
   改动,这次没做,Weekly 只是比 Daily 诚实一点的退让,不是精确阈值门。
   这是 BW 调度器一个真实的产品能力缺口,留给以后。

真实执行验证(不是描述,是真跑的命令 + sqlite 读回):
```
...
sqlite3 practice-aihot/bw-aihot.db \
  "SELECT name, schedule, status, mode, issue_stage FROM cron_task;"
# aihot 每日日报生成       | daily  | paused | create_issue | build
# aihot 治理复盘 · 按需开发  | weekly | normal | create_issue | optimize
```
重跑 `cron` 子命令验证幂等:老任务已 Paused 不会重复暂停,新任务已存在直接
跳过打印,`cron_task` 表行数保持 2,不重复造。

**运行态(每天真实产出日报 + 写 telemetry.json)刻意不建成 BW cron**——这是
一个真实的架构判断,不是遗漏。`aihot/main.py` 是确定性脚本(拉取→打分→
去重→渲染→写 telemetry),不是 agent 创作;BW 的 `CronTask` 今天只有两种
mode——`RunWorkflow`(真的会走 `claude -p` 执行器、真花钱)和 `CreateIssue`
(建活)——都不是"跑一个确定性脚本"的正确语义。如果把这个脚本包成一个
`WorkflowSpec` 挂进 Hub,用户点「▶ 运行」就会真实触发 agent 去"重新实现"
脚本已经做完的事——这是一个会误导、还会真花钱的假东西,所以**没有**把它
注册进 workflow_spec 表。真实的运行态自动化留在它本来的位置:用户自己机器
上的 OS 级调度(crontab/launchd)每天调用 `python -m aihot.main`,再调
已有的 `feed-telemetry` 子命令(K4 交付)把产出真实喂进 BW 的命中率/连续
产出天数指标——`cmd_cron` 的文档注释里留了一份真实可抄的 crontab 示例。
这条"缺口"如实记在这里,不是含糊带过。

**指标三层最终口径(答用户"issue 解决数不完全是引领/滞后指标"的拷问)**:
K4 已经把 issue 计数(`本周结算活数`)的 `def` 改成诚实的"工作量参考,非
产品引领指标"框架,L5 没有再改指标定义,只是把这条拷问的完整回答写清楚:
**引领**(命中率/关注面覆盖度,预示未来)→**滞后**(连续产出天数/累计日报,
已发生的结果)→**工作量/过程**(issue 数/commit 数,衡量"我改了多少"而非
"产品对用户好不好")。看板判健康只用前两层;工作量层留作过程参考,
不参与 `recompute_signals` 的健康聚合(前面 K4 已验证过这点)。

## 6. 证据归档(2026-07-22)

`practice-aihot/`(真实 DB + 带嵌套 `.git` 的真实项目工作区)按 `.gitignore` 的既有
决定不进本仓——不是源码,且嵌套 `.git` 会造成 gitlink 混乱。为了让这次践行的数字侧
证据可长期核验、不只活在某次践行者的本地磁盘上,新增 `crates/bw-app/examples/
archive_aihot_evidence.rs`:只读、零 mock,把 `Store` 真实查询 + 工作区真实 git 证据
(`bw_engine::evidence::collect`)拼成一份可提交的 JSON 快照,写入
[`iterations/AIHOT-EVIDENCE.json`](AIHOT-EVIDENCE.json)。本文件是叙事汇总,那份
JSON 是它的数字侧存档,两者互补不重复;独立核验:

```
cargo run -p bw-app --example archive_aihot_evidence -- \
    <bw-aihot.db 路径> <aihot 工作区路径> iterations/AIHOT-EVIDENCE.json
```

归档时读回:31 个 Issue(29 Done/1 InProgress/1 Todo)、4 次 workflow_run(1 Ok/3
