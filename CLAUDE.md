...
2. **活让 agent 干,人守验收门**。一件活就是一张 Issue 卡,指派给 AI 队友后一键真实开工(`RunIssue` 命令,在项目的真实目录里改文件、跑测试)。队友干完,活最远只能走到「评审中」;**「完成」永远由人显式点**——状态机里 Done 的唯一入口就是「评审中」,由 `can_transition_to` 在代码层面锁死。干砸了就如实停在原地,可以重试,不假装前进。定时任务到点要么真的去干(运行工作流/技能/Prompt),要么只留痕不动手(自动建活的 Autopilot 模式、采集观测),但绝不自动**完成**活。
...
**门禁(每个 commit 前全过;「门禁」=提交前必须全部通过的检查组。本地清单与 CI 不完全一致:CI 侧还跑 `cargo test`,差异以 `.github/workflows/ci.yml` 为准)**:
...
这个仓库最大的风险不是编译不过,而是**做出徒有其形的东西**:面板渲染了但数字是编的、流程走通了但记账没发生。以下纪律定义了本仓库里"真实"的操作含义。**2026-07-17 起核心纪律转向:行为正确性靠 E2E(computer-use:深链启动 + screencapture + sqlite 读回)+ `/code-review` 把质量;产品铁律由类型与守卫在编译期守住,E2E 读回抽查。**(修订:V1 起内核层重新带有少量单元测试,CI 的 `cargo test` 会跑——见 CONTEXT.md「门禁」词条;但不建永久累积的回归大坝,E2E 仍是行为正确性的主要手段。)
