...
**这一段不是借的,是这里自己写的**:认组合键复制粘贴。

> **2026-08-21 复核纠正**:下面那句「这个壳没有原生菜单」**是错的**。Dioxus 0.7.9
> 在没人调 `with_menu` 时会自己装一份默认菜单栏,里面就有带 copy / paste 的
> Edit 子菜单(`dioxus-desktop-0.7.9/src/config.rs:48` → `menubar.rs:82`),而这个
> 壳的窗口装饰是开着的,所以那份默认菜单确实生效。**用户报「复制不出来」更可能
> 的根因是压根没选中**:claude 的 TUI 开着鼠标上报,拖拽被当成鼠标事件送给了程序,
> 不产生选区。这一条**没有在真机上证实过**,如实标注。
>
> 那么这一段还留着做什么?两个理由都成立:①它顺手修掉了 `keyBytes` 那个真 bug
> (Cmd+C 会往 PTY 里凭空敲一个 `c`);②标题栏那颗「复制」按钮**没选中时复制整段**
> ——不管选不选得中,人总有一条路把内容拿出来。
