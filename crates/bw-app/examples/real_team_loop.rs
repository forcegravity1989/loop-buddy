...
async fn main() {
    ...
    app.dispatch(Command::CreateProject {
        ...
        desc: "用 BW 自己的五角色环,把 multica「真实 agent 队友」融进 BW「五阶段方法论 + 度量派生」".into(),
    }).await.unwrap();
    app.dispatch(Command::SetCycle { cycle: ProjectCycle::Explore }).await.unwrap();
    app.dispatch(Command::CompleteCreation { cadence: Cadence::Weekly }).await.unwrap();
    ...
    app.dispatch(Command::CreateAgent {
        id: fable, name: "Fable · 调度(原型/运营/运维)".into(),
        role: "PM + 环协调".into(), skills: vec![], model: "fable".into(),
    }).await.unwrap();
    let sonnet_builder = AgentId::new();
    app.dispatch(Command::CreateAgent {
        id: sonnet_builder, name: "sonnet5 · 构建师".into(),
        role: "构建".into(), skills: vec![], model: "sonnet".into(),
    }).await.unwrap();
    ...
    for w in &works {
        ...
        app.dispatch(Command::CreateIssue {
            id: iid, stage: w.stage, title: w.title.into(), desc: w.desc.into(), priority: w.priority,
        }).await.unwrap();
        let assignee = if w.agent_name.starts_with("sonnet") { sonnet_builder } else { fable };
        app.dispatch(Command::AssignIssue { id: iid, assignee: Some(assignee) }).await.unwrap();
        app.dispatch(Command::TransitionIssue { id: iid, status: IssueStatus::Done }).await.unwrap();
        issue_ids.push((iid, w));
        println!("   · [{}]「{}」", w.stage.label(), w.title);
        println!("     assignee = {}({}) · 状态 = Done · 证据 = {}", w.agent_name, w.agent_role, w.evidence);
    }
    ...
    app.dispatch(Command::HandoffStage {
        risky: true,
        note: "构建→优化:R1/R2/App集成全绿(140 测试)。险交棒:优化段 DoD「压测证据/预算全绿」未达成。".into(),
    }).await.unwrap();
    ...
    app.dispatch(Command::DistillSkillFromIssue {
        skill_id: skill, issue_id: distill_from,
        name: "五角色环 · 真实交付法".into(),
        desc: "从一件真实完成的 Issue 蒸馏:五角色各一轮、门禁每圈绿、证据可核验、不 mock。".into(),
        category: "方法论".into(),
    }).await.unwrap();
    app.dispatch(Command::RefreshHubs).await.unwrap();
    let distilled = app.snapshot().skills.iter().find(|s| s.distilled_from_issue.is_some()).unwrap();
    ...
    let done = issues.iter().filter(|i| i.status == IssueStatus::Done).count();
    let total = issues.len();
    let skills_with_provenance = app.snapshot().skills.iter().filter(|s| s.distilled_from_issue.is_some()).count();
    ...
    println!("  完成率(Done/总)= {:.0}%", (done as f32 / total as f32) * 100.0);
    println!("  带真实 provenance 的 Skill = {}", skills_with_provenance);
    println!("  真实交棒次数(审计)= {}", handoffs);
    println!("  活跃阶段 = {} · 阶段环已推进 2 段", proj.active_stage.label());
    println!("╚══════════════════════════════════════════╝\n");
