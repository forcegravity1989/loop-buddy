...
// ─────────────── V1 Issue 1 · CodeHub repo summary VM ───────────────
//
// `CodehubRepoSummaryVm` is the UI-layer projection of a codehub repo row
// (对仗 `bw_engine::github::GithubRepoSummary` which app-desktop consumes
// directly for the github side — this VM gives the codehub side the same pure,
// serializable DTO that app-desktop can build from `bw_engine::codehub::
// CodehubRepoSummary` without the `ui` crate taking a `bw-engine` dependency).
// `pushed_at` is codehub's `last_activity_at` (same semantic — last real
// activity on the repo), surfaced as a pre-formatted label for the picker.

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct CodehubRepoSummaryVm {
    /// `path_with_namespace` (e.g. `innersource/AI-Coding_G/maas`).
    pub path: String,
    /// `private` / `public` / `internal`.
    pub visibility: String,
    /// Default branch name (empty = no commits yet / edge case).
    pub default_branch: String,
    /// Last activity ISO timestamp (empty = not returned).
    pub pushed_at: String,
    /// Repo description (empty = repo has none set).
    pub description: String,
}

/// Pure builder: maps the raw field values (from `bw_engine::codehub::
/// CodehubRepoSummary`) into the VM. `app-desktop` calls this in its kernel
/// build (same place it passes `github_repos` through to the `Vm`).
pub fn codehub_repo_summary_vm(
    path: &str,
    visibility: &str,
    default_branch: &str,
    pushed_at: &str,
    description: &str,
) -> CodehubRepoSummaryVm {
    CodehubRepoSummaryVm {
        path: path.to_string(),
